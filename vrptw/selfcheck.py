"""
Быстрая самопроверка стенда — первое, что стоит запустить.

Проходит весь путь: граф, генерация дня, оба планировщика, проверка
инвариантов плана, симуляция, выгрузка для интерфейса и проверка
контракта. Около полутора минут вместо пятнадцати у полных замеров.

Если здесь всё зелёное — стенд собран правильно, и длинные замеры
можно запускать с доверием.

Запуск:  python3 -m vrptw.selfcheck
"""

from __future__ import annotations

import json
import re
import shutil
import sys
import tempfile
import time
from pathlib import Path

OK = "  ✓"
FAIL = "  ✗"

# Оба списка — уровня модуля, и `main` обязан их чистить: иначе два
# вызова `main()` в одном процессе покажут ошибки первого во втором.
# На продукте это не видно (проверка живёт один процесс), но цена
# незамеченной ошибки тут — ложное «всё хорошо».
failures: list[str] = []
отметок = 0


def step(title: str):
    print(f"\n{title}")


def ok(msg: str):
    global отметок
    отметок += 1
    print(f"{OK} {msg}")


def bad(msg: str):
    print(f"{FAIL} {msg}")
    failures.append(msg)


def _cascade_day(net):
    """
    День из двух заявок, на котором снятие визита ломает хвост маршрута.

    Выезд из дома в 8:57 попадает в утренний пик, выезд в 9:01 — уже нет,
    и та же дорога занимает на десять минут меньше. Поэтому короткий
    первый визит маршрут не задерживает, а спасает: без него инженер
    выезжает раньше и приезжает позже, чем закрывается окно второй
    заявки. Время в пути немонотонно по часу выезда — вот на чём это
    держится.

    На синтетическом дне такое не выпадает: 70 тысяч выбиваний и
    исчерпывающий перебор всех снятий не дали ни одного случая. Поэтому
    ветка каскада проверяется здесь, а не ждёт настоящих данных.
    """
    from .core.domain import Day, Engineer, Order

    shift_start = 8 * 60 + 57
    depart_after = shift_start + 4          # 3 минуты «последней мили» плюс визит на минуту

    # Узел, до которого разница между выездом в пик и сразу после него
    # больше, чем длится снятый визит.
    node = max(range(1, len(net.nodes)),
               key=lambda b: (net.travel_minutes(0, b, shift_start)
                              - net.travel_minutes(0, b, depart_after)))
    late = int(round(net.travel_minutes(0, node, depart_after)))

    def order(oid, at_node, window_end, est):
        return Order(id=oid, work_type="repair", district="—", lat=0.0, lon=0.0,
                     node=at_node, window_start=shift_start, window_end=window_end,
                     sla_deadline=22 * 60, priority=0, est_minutes=est)

    first = order("A0", 0, shift_start + 60, 1)
    # Окно второй заявки закрывается ровно в момент прибытия с первой.
    second = order("B0", node, depart_after + late, 30)
    eng = Engineer(id="E00", name="—", skills={"repair"}, grade=2, home_node=0,
                   home_lat=0.0, home_lon=0.0, shift_start=shift_start,
                   shift_end=20 * 60, speed=1.0)
    return Day(date="каскад", orders=[first, second], engineers=[eng], network=net)


# Отпечаток потока аварий на московском пуле и на синтетической сети —
# снят на коде до правки 21 сентября (дом зоны, `urgent_near`). Правка
# трогала только сети зон; здесь поток обязан остаться прежним до байта,
# иначе сдвинутся все замеры пересчёта на синтетике.
ПОТОК_АВАРИЙ = {"москва": "fd77b8f52c5815f1", "синтетика": "88b289d425527900"}


def _поток_аварий(net, срочные=None) -> str:
    """Отпечаток аварий на синтетических днях: зёрна 1, 2, 7, в 10:00 и 13:40."""
    import hashlib
    from .core.generate import generate_day
    from .replan import urgent_orders
    срочные = срочные or urgent_orders
    поток = [(s, at, o.node, o.district, round(o.lat, 6), round(o.lon, 6), o.address)
             for s in (1, 2, 7) for at in (600, 820)
             for o in срочные(generate_day(seed=s, network=net), at, 10, s)]
    return hashlib.sha1(repr(поток).encode()).hexdigest()[:16]


def _аварии_на_зонах(дни: dict, срочные=None) -> tuple[list[str], list[str]]:
    """
    Где встают аварии на сетях зон: что не так с разбросом по домам и что
    не так с привязкой к инженеру. Пустые списки — всё в порядке.

    Прежде адрес аварии брался по московскому району, района в зоне не
    было, и вставал ближайший к его центру дом: на юго-востоке 51 авария
    из 80 падала в один дом, на всех зонах адрес был «None, None», одна —
    в точку выезда. Порог — 25 домов из 80 и не больше 10 в одном: прежнее
    поведение давало 7–20 домов и 19–51 в одном, нынешнее — 43–50 и 4–6.
    """
    from collections import Counter
    from .replan import urgent_orders
    срочные = срочные or urgent_orders
    разброс: list[str] = []
    привязка: list[str] = []
    for з, д in дни.items():
        адреса: dict[int, set] = {}
        for o in д.orders:
            адреса.setdefault(o.node, set()).add(o.address)
        ав = срочные(д, 820, 80, 0)
        c = Counter(o.node for o in ав)
        чужие = sum(o.node not in адреса for o in ав)
        без_адреса = sum(not o.address or o.address not in адреса.get(o.node, ()) for o in ав)
        if len(c) < 25 or c.most_common(1)[0][1] > 10 or чужие or без_адреса:
            разброс.append(f"{з}: домов {len(c)}, в одном до {c.most_common(1)[0][1]}, "
                           f"не в доме заявки {чужие}, с чужим адресом {без_адреса}")
        # «У кого на участке»: восемь ближайших по дороге домов заявок к
        # точке, где инженер сейчас. Восемь — договорённость из CONTRACT.md,
        # здесь она записана числом, а не взята из replan, чтобы сторож
        # не двигался вместе с кодом.
        около = д.engineers[0].home_node
        сеть = д.network
        ближ = set(sorted((n for n in адреса if n != около),
                          key=lambda n: (сеть.road_km_between(около, n), n))[:8])
        try:
            рядом = {o.node for o in срочные(д, 820, 20, 0, near_node=около)}
        except TypeError as e:
            привязка.append(f"{з}: {e}")
            continue
        if not рядом <= ближ or len(рядом) < 3:
            привязка.append(f"{з}: домов {len(рядом)}, вне восьми ближайших {len(рядом - ближ)}")
    return разброс, привязка


# Как называются данные. Правило 12: выгрузка обезличенная и выдана
# организаторами. Само слово «заказчик» законно — это роль бизнеса в
# предметной области («ТЗ заказчика», «цифра для заказчика»); запрещены
# связки, где заказчику приписаны ДАННЫЕ. До 23 сентября таких связок
# было тринадцать в одном `CONTRACT.md`, который лежит в публичном снимке.
_ЧУЖИЕ_ДАННЫЕ = (
    "данные заказчика", "данных заказчика", "данным заказчика",
    "выгрузка заказчика", "выгрузки заказчика", "выгрузке заказчика",
    "зона заказчика", "зоны заказчика", "зоне заказчика",
    "у заказчика",
)

# Что читает проверяющий. Записки и замеры сюда не входят: они говорят о
# прошлом и вправе цитировать прежние формулировки.
_ДОКУМЕНТЫ_НАРУЖУ = (
    "README.md", "ФУНКЦИОНАЛЬНЫЕ-ТРЕБОВАНИЯ.md",
    "vrptw/CONTRACT.md", "vrptw/README.md",
)

# Замеры тоже читают: часть кругов едет в снимок, и именно из них берут
# числа для презентации. Смотрим сводки и выводы — то, что человек
# открывает; комментарии в скриптах замеров, как и в движке, остаются
# рабочими заметками.
#
# Исключение — замороженные слепки нашего собственного журнала
# (`CLAUDE-до-правки.md` и подобные): они лежат в замерах нарочно, чтобы
# показать, что документ говорил тогда. Править их значит стереть то
# единственное, ради чего они сохранены.
_ЗАМЕРЫ_НАРУЖУ = ("*.md", "*.txt")
_НЕ_ПРАВИМ_ИСТОРИЮ = "CLAUDE"


def _контрольные_не_читаются(корень: Path | None = None) -> tuple[list[str], int]:
    """Контрольные выгрузки не попадают ни в один путь исполнения.

    Правило 12 держалось на чтении кода, а раздел 6 ФТ обещал проверку.
    Вот она, тремя вопросами:

    * загрузчик знает ровно три файла заявок — `REGIONS`;
    * никто не обходит каталог заявок маской: иначе новый файл рядом
      подхватился бы молча, и «мы их не читаем» держалось бы на том, что
      никто не положил лишнего;
    * имена посторонних файлов из этого каталога не встречаются в
      исходниках движка.

    «Посторонний» считается от `REGIONS`, а не по слову «контроль»:
    так же поймается любой другой файл, который окажется рядом.

    Второе число — сколько посторонних файлов лежит в каталоге. **Ноль —
    это не провал, а лучшее из состояний**: в публичном снимке
    контрольных выгрузок нет вовсе, и первые две проверки продолжают
    работать. Выяснилось на самом снимке: сторож требовал, чтобы
    посторонние файлы нашлись, и падал ровно там, где их правильно нет.
    """
    from .core.load import ORDERS_DIR, REGIONS

    плохо: list[str] = []
    if len(REGIONS) != 3:
        плохо.append(f"загрузчик знает {len(REGIONS)} зон, а их три")
    известные = {Path(п).name for п in REGIONS.values()}
    посторонние = sorted(p.name for p in ORDERS_DIR.iterdir()
                         if p.is_file() and p.name not in известные)

    корень = корень or Path(__file__).resolve().parent.parent
    # Сам сторож каталог обходит — на то он и сторож; его не считаем.
    свой = Path(__file__).name
    for путь in sorted(корень.rglob("*.py")):
        if "__pycache__" in путь.parts or путь.name == свой:
            continue
        текст = путь.read_text(encoding="utf-8")
        for обход in ("ORDERS_DIR.glob", "ORDERS_DIR.rglob",
                      "ORDERS_DIR.iterdir", "ORDERS_DIR.walk"):
            if обход in текст:
                плохо.append(f"{путь.name}: {обход} — каталог заявок обходится "
                             f"маской, лишний файл подхватится молча")
        for имя in посторонние:
            if имя in текст:
                плохо.append(f"{путь.name}: упомянут {имя}")
    return плохо, len(посторонние)


def _порог_приоритета(корень: Path | None = None) -> tuple[list[str], int]:
    """Документы обязаны называть то число, которое даёт `PRIORITY_WEIGHTS`.

    Ступени приоритета разведены весами, а не отдельными уровнями
    сравнения: при 1 : 100 : 10 000 одна авария дороже 99 подключений, но
    не дороже ста. До 23 сентября README, ФТ и `CLAUDE.md` обещали «хуже
    любого числа» — на зонах (56, 66 и 83 заявки) разницы нет, а в чужой
    выгрузке со ста с лишним подключениями обещание перестаёт быть
    правдой.

    Здесь число считается из самих весов и сверяется с документами
    наружу: поменяются веса — проверка потребует поправить и слова.
    Возвращает (нарушения, порог).
    """
    from .solvers.improved import PRIORITY_WEIGHTS

    пороги = {int(старший // младший) - 1
              for младший, старший in zip(PRIORITY_WEIGHTS, PRIORITY_WEIGHTS[1:])
              if младший}
    корень = корень or Path(__file__).resolve().parent.parent
    плохо: list[str] = []
    if len(пороги) != 1:
        плохо.append(f"ступени разведены неодинаково: пороги {sorted(пороги)} — "
                     f"одним числом это уже не описать")
        return плохо, 0
    порог = пороги.pop()

    # Обещание «хуже любого числа» весами не обеспечивается никогда.
    запрещено = ("любого числа нераспределённых", "любое число нераспределённых",
                 "любого числа ремонтов")
    for имя in _ДОКУМЕНТЫ_НАРУЖУ:
        путь = корень / имя
        if not путь.is_file():
            continue
        текст = путь.read_text(encoding="utf-8")
        низ = текст.lower()
        for фраза in запрещено:
            if фраза in низ:
                плохо.append(f"{имя}: «{фраза}» — весами это не держится")
        # Где про приоритет говорят, там обязаны назвать порог — и
        # именно рядом с фразой, а не где-нибудь в файле. Раньше стояло
        # `str(порог) not in текст`, то есть искалось вхождение цифр по
        # всему документу: при весах (1, 7, 49) порог равен 6, документ
        # по-прежнему говорил «99», а сторож молчал — цифра «6» в тексте
        # где-нибудь да найдётся.
        рядом = re.findall(r"(\d+)\s+нераспределённых", текст)
        if рядом and any(int(ч) != порог for ч in рядом):
            плохо.append(f"{имя}: сказано «{рядом[0]} нераспределённых», "
                         f"а веса дают порог {порог}")
        if "приоритет" in низ and "нераспределённая авария" in низ \
                and not рядом:
            плохо.append(f"{имя}: про приоритет сказано, а порог {порог} "
                         f"не назван")
    return плохо, порог


# Что едет в публичный снимок. Список здесь, а не только прозой в
# `CLAUDE.md`: по нему работает сторож ниже, и разъехаться им нечем.
# Рабочих записок, `CLAUDE.md`, передач и контрольных выгрузок в снимке
# нет и быть не должно.
# Чем помечается честная ссылка на то, чего в снимке нет.
_ОГОВОРКИ = ("в снимок не", "снимок не входит", "рабочем репозитории",
             "рабочий репозитор")

_СНИМОК = (
    "vrptw", "web", "frontend_kit",
    "замеры-21-09", "замеры-22-09", "замеры-22-09-вечер", "замеры-24-09",
    "README.md", "ФУНКЦИОНАЛЬНЫЕ-ТРЕБОВАНИЯ.md", "requirements.txt",
    ".gitignore", ".gitattributes",
    "собрать-интерфейс.sh", "проверка-с-нуля.sh",
    # Обёртки для Windows: там нет ни `python3`, ни `zsh`, а первая
    # команда README открыла бы Microsoft Store. Проверяющий должен
    # получить их вместе со снимком, иначе советовать их незачем.
    "проверка.bat", "запустить.bat", "поставить.bat",
)


def _ссылки_наружу(корень: Path | None = None) -> list[str]:
    """Документы наружу не должны ссылаться на то, чего в снимке нет.

    Ссылка на `CLAUDE.md` или на старый круг замеров в рабочем
    репозитории цела, а в снимке ведёт в пустоту: туда эти файлы не
    едут нарочно. Читающий идёт по ссылке и не находит ничего —
    и это первое, что он узнаёт о нашей аккуратности.

    Ловим только те ссылки, которые в репозитории **существуют**: если
    путь есть у нас и не попадает в снимок — это обрыв. Текст, похожий
    на путь, но не существующий (`дома.json`, `<вид>-<зона>.json`), не
    трогаем: это образец, а не ссылка.

    Чинить такую ссылку надо не удалением, а правдой: сказать, что файл
    живёт в рабочем репозитории и в снимок не входит. Тогда текст верен в
    обоих местах — и сторож такую оговорку принимает, иначе правило
    спорило бы само с собой. Оговорка должна стоять **во той же фразе**,
    что и ссылка, а не просто рядом: с окном в пару строк одна оговорка
    внизу абзаца оправдывала бы все ссылки выше — проверено, так и
    вышло. Фраза кончается точкой, точкой с запятой, пустой строкой или
    началом строки таблицы.
    """
    import re

    корень = корень or Path(__file__).resolve().parent.parent
    едет = set(_СНИМОК)
    плохие = []
    for имя in _ДОКУМЕНТЫ_НАРУЖУ:
        путь = корень / имя
        if not путь.is_file():
            continue
        текст = путь.read_text(encoding="utf-8")
        # Фраза — единица, в которой оговорка считается относящейся к
        # ссылке. Границы: точка, точка с запятой, пустая строка и начало
        # строки таблицы или списка.
        границы = re.compile(r"(?<=[.;])\s+|\n\s*\n|\n(?=\s*[|\-*])")
        начало = 0
        for кусок in границы.split(текст):
            if кусок is None:
                continue
            место = текст.find(кусок, начало)
            начало = место + len(кусок) if место >= 0 else начало
            строка_н = текст.count("\n", 0, max(место, 0)) + 1
            низ = кусок.lower()
            есть_оговорка = any(м in низ for м in _ОГОВОРКИ)
            for цель in re.findall(r"`([^`\s]+\.(?:md|py|sh|json|txt|csv)|"
                                   r"[^`\s]*/)`", кусок):
                цель = цель.strip("/")
                if not цель or not (корень / цель).exists():
                    continue
                if цель.split("/", 1)[0] in едет or есть_оговорка:
                    continue
                плохие.append(f"{имя}:{строка_н} → `{цель}` "
                              f"(в снимок не едет, оговорки во фразе нет)")
    return плохие


def _чьи_данные(корень: Path | None = None) -> list[str]:
    """Где данные приписаны заказчику — в документах наружу и в том, что
    движок говорит вслух.

    Двух документов мало. Первое, что видит проверяющий, — баннер
    сервера, и до 23 сентября он говорил «на данных заказчика: curl …».
    Туда же смотрят подписи ручек в `/api`. Поэтому сюда добавлены
    строковые литералы движка.

    **Граница проведена по тому, что человек видит.** Строка в коде
    печатается или уходит в ответ — её ловим. Комментарии и докстроки —
    рабочие заметки инженера, их 45 штук, и переписывать их мы не стали:
    правило 12 про то, как мы **называем данные перед людьми**, а не про
    то, каким словом обходимся между собой. Захотим — отдельным заходом,
    не накануне защиты.
    """
    import ast

    корень = корень or Path(__file__).resolve().parent.parent
    плохие = []

    def найти(текст: str) -> str | None:
        низ = текст.lower()
        return next((с for с in _ЧУЖИЕ_ДАННЫЕ if с in низ), None)

    for имя in _ДОКУМЕНТЫ_НАРУЖУ:
        путь = корень / имя
        if not путь.is_file():
            continue
        for n, строка in enumerate(путь.read_text(encoding="utf-8").splitlines(), 1):
            связка = найти(строка)
            if связка:
                плохие.append(f"{имя}:{n} «{связка}»")

    for каталог in sorted(корень.glob("замеры-*")):
        if not каталог.is_dir():
            continue
        for маска in _ЗАМЕРЫ_НАРУЖУ:
            for путь in sorted(каталог.rglob(маска)):
                if путь.name.startswith(_НЕ_ПРАВИМ_ИСТОРИЮ):
                    continue
                try:
                    строки = путь.read_text(encoding="utf-8").splitlines()
                except (UnicodeDecodeError, OSError):
                    continue
                for n, строка in enumerate(строки, 1):
                    связка = найти(строка)
                    if связка:
                        плохие.append(
                            f"{путь.relative_to(корень)}:{n} «{связка}»")

    пакет = корень / "vrptw"
    свой = Path(__file__).name
    for путь in sorted(пакет.rglob("*.py")) if пакет.is_dir() else []:
        # Себя не считаем: здесь лежит сам список связок.
        if "__pycache__" in путь.parts or путь.name == свой:
            continue
        try:
            дерево = ast.parse(путь.read_text(encoding="utf-8"))
        except (SyntaxError, UnicodeDecodeError, OSError):
            continue
        докстроки = set()
        for узел in ast.walk(дерево):
            if isinstance(узел, (ast.Module, ast.FunctionDef,
                                 ast.AsyncFunctionDef, ast.ClassDef)) and узел.body:
                первый = узел.body[0]
                if isinstance(первый, ast.Expr) and isinstance(первый.value, ast.Constant) \
                        and isinstance(первый.value.value, str):
                    докстроки.add(id(первый.value))
        for узел in ast.walk(дерево):
            if not (isinstance(узел, ast.Constant) and isinstance(узел.value, str)):
                continue
            if id(узел) in докстроки:
                continue
            связка = найти(узел.value)
            if связка:
                плохие.append(f"{путь.relative_to(корень)}:{узел.lineno} «{связка}»")
    return плохие


def _голые_assert(корень: Path | None = None) -> list[str]:
    """Проверки инвариантов, которые выключаются флагом интерпретатора.

    Голый `assert` исчезает под `python3 -O`. `require_valid` для того и
    заведён: он бросает `InvalidPlan`, и отключить его нечем.

    До 23 сентября сторож смотрел только внутрь `vrptw/` и сличал строки
    регулярным выражением. Он не видел ни замеров — а они лежат рядом и
    тоже печатают числа в документы, — ни записи, разнесённой на две
    строки. Голых `assert` в замерах оказалось девять, и среди них
    `замеры-22-09-вечер/потолок-зон.py`: тот самый, которым сняты числа
    предела в ФТ и README. Теперь разбор по дереву и по всему
    репозиторию, где перенос строки ничего не меняет.
    """
    import ast

    корень = корень or Path(__file__).resolve().parent.parent
    плохие = []
    for путь in sorted(корень.rglob("*.py")):
        if "__pycache__" in путь.parts or "node_modules" in путь.parts:
            continue
        try:
            дерево = ast.parse(путь.read_text(encoding="utf-8"))
        except (SyntaxError, UnicodeDecodeError, OSError):
            # Файл, который не разбирается, сторож и не покрывает. Сейчас
            # таких нет; появится — увидим по этой же строке в обходе.
            continue
        # Имена, которым присвоен результат `check_invariants`. Без них
        # сторож видел только прямую запись `assert not
        # plan.check_invariants(day)`, а канонический замерный вид —
        # через переменную —
        #
        #     проблемы = план.check_invariants(день)
        #     assert not проблемы, проблемы[:2]
        #
        # проходил мимо: в `test` там одно имя `проблемы`, и ни
        # `check_invariants`, ни `require_valid` в нём нет. Таких мест
        # нашлось 29, из них шесть едут в снимок — включая
        # `приоритет.py` и `рекомендация-честно.py` круга 24 сентября,
        # то есть скрипты, которыми сняты числа в ФТ и наверху
        # `CLAUDE.md`. Под `python3 -O` вызов остаётся, но его результат
        # перестаёт что-либо значить.
        от_инварианта: set[str] = set()
        for узел in ast.walk(дерево):
            if isinstance(узел, ast.Assign) and isinstance(узел.value, ast.Call):
                ф = узел.value.func
                если_имя = getattr(ф, "attr", None) or getattr(ф, "id", None)
                if если_имя in {"check_invariants", "require_valid"}:
                    for цель in узел.targets:
                        if isinstance(цель, ast.Name):
                            от_инварианта.add(цель.id)

        for узел in ast.walk(дерево):
            if not isinstance(узел, ast.Assert):
                continue
            имена = {n.attr for n in ast.walk(узел.test)
                     if isinstance(n, ast.Attribute)}
            имена |= {n.id for n in ast.walk(узел.test) if isinstance(n, ast.Name)}
            if имена & ({"check_invariants", "require_valid"} | от_инварианта):
                плохие.append(f"{путь.relative_to(корень)}:{узел.lineno}")
    return плохие


def _боевое_состояние(каталог: Path) -> tuple[str, int]:
    """Отпечаток боевых файлов состояния: архив, сравнения, журнал,
    настройки, сохранённые пересчёты, загруженные выгрузки.

    Проверка не имеет права их трогать (правило 9). Сейчас она и не
    трогает — замерено подстановкой чужого журнала, архива и настроек:
    вывод совпал строка в строку, значит и не читает. Но держалось это
    на том, что никто не позвал `Journal.load`, `save` или `Stand`:
    `Journal(day="1")` в проверке — конструктор, а не чтение с диска, и
    одна буква разницы превратила бы самопроверку в правку боевого
    журнала накануне защиты.

    Поэтому каталог состояния уводится в сторону, а этот отпечаток
    сторожит то, что уводом не лечится: путь, зашитый мимо `state.py`.
    """
    import hashlib
    h = hashlib.sha1()
    сколько = 0
    for путь in sorted(каталог.glob(".vrptw-*")):
        файлы = ([путь] if путь.is_file()
                 else [f for f in sorted(путь.rglob("*")) if f.is_file()])
        for f in файлы:
            h.update(f.name.encode("utf-8"))
            h.update(f.read_bytes())
            сколько += 1
    return h.hexdigest()[:16], сколько


def _отпечаток_данных() -> tuple[str, int]:
    """Отпечаток всего `vrptw/data` — чем он был до проверки и стал после.

    Проверка ничего не меняет и не должна оставлять следов в
    репозитории. Нарушалось это молча: при интернете выгрузка форм
    складывала добытые ломаные в `vrptw/data/moscow_geometry.json`, и
    после каждого прогона у файла оказывалась своя горсть перегонов —
    план в проверке считается по бюджету в секундах, маршруты выходят
    чуть разные. Ловилось только глазами, в `git status`.

    Считается по содержимому, а не по времени правки: атомарная запись
    идёт через временный файл и `os.replace`, и файл с тем же
    содержимым получил бы новое время.
    """
    import hashlib
    корень = Path(__file__).resolve().parent / "data"
    h = hashlib.sha1()
    сколько = 0
    for путь in sorted(корень.rglob("*")):
        if путь.is_file():
            h.update(путь.relative_to(корень).as_posix().encode("utf-8"))
            h.update(путь.read_bytes())
            сколько += 1
    return h.hexdigest()[:16], сколько


def _досягаемость_в_причинах(дни: dict, срочные=None) -> tuple[list[str], int]:
    """
    Причина «у всех занято более срочными» не должна доставаться заявке,
    до которой никто не успевает доехать.

    Так было до 23 сентября: на юго-востоке авария в 7:00 в Москве
    получала «подходящих инженеров 1, но у всех это время занято», хотя
    единственный аварийщик участка выезжает из Каширы и на месте был бы в
    11:19 при окне до 10:00. Диспетчеру это разные новости: занятость
    лечится переброской заявок, расстояние — нет.

    Лучший случай считается здесь заново, а не берётся у `export`: сторож
    не должен двигаться вместе с кодом, который сторожит.

    Второе число — сколько недосягаемых случаев нашлось на живых данных.
    До 24 сентября их было два, и оба делал каширский выездной: он один
    оказывался далеко от московской аварии. Выездного убрали (разрешения
    на вторую стартовую точку не было, см. `_одна_точка_выезда`), и с
    общим офисом недосягаемых на зонах не осталось ни одного — до любой
    аварии участка успевает доехать хотя бы один аварийщик на машине.

    Поэтому «сколько нашлось» больше не условие: ноль теперь законный
    ответ живых данных. Ветку проверяет `_ПРОБА_ДОСЯГАЕМОСТИ` на
    построенных случаях — правило 8 ровно для этого и написано.
    """
    from .core.domain import TRANSPORT
    from .export import _why_unassigned
    from .replan import urgent_orders
    срочные = срочные or urgent_orders
    плохо: list[str] = []
    недосягаемых = 0
    for з, д in дни.items():
        сеть = д.network
        for час in range(6, 21):
            авария = срочные(д, час * 60, 1, 0)[0]
            кандидаты = [e for e in д.engineers
                         if e.can_do(авария)
                         and e.shift_start < авария.window_end
                         and e.shift_end > авария.window_start]
            if not кандидаты:
                continue
            успевает = False
            for e in кандидаты:
                предел = TRANSPORT[e.transport]["max_km"]
                км = сеть.road_km_between(e.home_node, авария.node)
                if предел is not None and км > предел:
                    continue
                ехать = сеть.travel_minutes(e.home_node, авария.node,
                                            e.shift_start, e.transport)
                if e.shift_start + round(ехать) <= авария.window_end:
                    успевает = True
                    break
            причина = _why_unassigned(д, авария, None, None)
            про_дорогу = "не успевает доехать" in причина or "не доберётся" in причина
            if not успевает:
                недосягаемых += 1
                if not про_дорогу:
                    плохо.append(f"{з}, {час}:00 — доехать некому, а сказано "
                                 f"«{причина}»")
            elif про_дорогу:
                плохо.append(f"{з}, {час}:00 — доехать успевают, а сказано "
                             f"«{причина}»")
    return плохо, недосягаемых


# Где у известных текстовых вызовов стоят «режим» и «кодировка» среди
# позиционных и какой режим у них по умолчанию:
#
#     имя -> (место режима, место кодировки, режим по умолчанию)
#
# `None` на месте режима — режима у вызова нет, он всегда текстовый;
# `None` на месте кодировки — её туда можно передать только по имени.
#
# Таблица нужна потому, что места разные: у встроенного `open` режим
# второй, у `Path.open` первый, у `SpooledTemporaryFile` снова второй.
# Пока место считалось одно на всех, `gzip.open(p, "rb")` и
# `io.open(p, "rb")` попадали в список зря — за режим сторож принимал имя
# файла. И режим по умолчанию у них разный: `open` без режима читает
# текст, а `NamedTemporaryFile` и `gzip.open` без режима двоичные.
_ТЕКСТОВЫЕ: dict[str, tuple[int | None, int | None, str]] = {
    "read_text": (None, 0, "r"),
    "write_text": (None, 1, "w"),
    "NamedTemporaryFile": (0, 2, "w+b"),
    "TemporaryFile": (0, 2, "w+b"),
    "SpooledTemporaryFile": (1, 3, "w+b"),
    "fdopen": (1, 3, "r"),
}

# `open` — одно имя на несколько разных вызовов; различаем по тому, на чём
# его зовут. Пустой ключ — встроенный `open`.
_ОТКРЫТИЯ: dict[str, tuple[int | None, int | None, str]] = {
    "": (1, 3, "r"),
    "io": (1, 3, "r"),
    "gzip": (1, 3, "rb"),
    "bz2": (1, 3, "rb"),
    "lzma": (1, None, "rb"),
    "codecs": (1, 2, "r"),
}

# Вызовы `subprocess`. Текстовый режим там включают `text`,
# `universal_newlines` и — чего сторож не знал — сам по себе `errors`:
# с ним потоки декодируются по локали, то есть на Windows в cp1251.
_ПОДПРОЦЕСС = {"run", "Popen", "call", "check_call", "check_output"}


def _ПРОБА_ДОСЯГАЕМОСТИ(день) -> list[str]:
    """
    Три построенных случая: сторож обязан различать дорогу и занятость.

    На живых зонах недосягаемых случаев не осталось (см.
    `_досягаемость_в_причинах`), а ветка есть, и сломать её можно молча.
    Случаи строятся из настоящего дня, а не из выдуманного: те же сеть,
    бригада и расстояния, меняются только узел заявки и окно.

      1. дальний узел и окно, закрывающееся сразу, — «не успевает доехать»;
      2. в смене один пешеход, а заявка за 107 км — «не доберётся»;
      3. ближний узел и широкое окно — причина НЕ про дорогу.

    Третий случай нужен не меньше первых двух: сторож, который на всё
    отвечает «далеко», прошёл бы обе первые проверки.
    """
    import dataclasses
    from .export import _why_unassigned
    from .replan import urgent_orders

    сеть = день.network
    офис = день.engineers[0].home_node
    образец = urgent_orders(день, 7 * 60, 1, 0)[0]
    узлы = range(len(сеть.nodes))
    дальний = max(узлы, key=lambda n: сеть.road_km_between(офис, n))
    ближний = min((n for n in узлы if n != офис),
                  key=lambda n: сеть.road_km_between(офис, n))
    ранний = min(e.shift_start for e in день.engineers
                 if образец.skill in e.skills)

    def дорога(текст: str) -> bool:
        return "не успевает доехать" in текст or "не доберётся" in текст

    плохо: list[str] = []

    далеко = dataclasses.replace(образец, node=дальний,
                                 window_start=ранний, window_end=ранний + 30)
    причина = _why_unassigned(день, далеко, None, None)
    if "не успевает доехать" not in причина:
        плохо.append(f"далёкая авария с закрытым окном: «{причина}»")

    пешие = [e for e in день.engineers if e.transport == "foot"]
    if not пешие:
        плохо.append("в бригаде нет пешехода — случай 2 не построить")
    else:
        пеший = dataclasses.replace(пешие[0],
                                    skills=set(пешие[0].skills) | {образец.skill})
        один = dataclasses.replace(день, engineers=[пеший])
        не_дойдёт = dataclasses.replace(образец, node=дальний,
                                        window_start=пеший.shift_start,
                                        window_end=пеший.shift_end)
        причина = _why_unassigned(один, не_дойдёт, None, None)
        if "не доберётся" not in причина:
            плохо.append(f"пешеход за 107 км: «{причина}»")

    рядом = dataclasses.replace(образец, node=ближний,
                                window_start=ранний, window_end=ранний + 240)
    причина = _why_unassigned(день, рядом, None, None)
    if дорога(причина):
        плохо.append(f"досягаемая авария объяснена дорогой: «{причина}»")

    return плохо


def _объяснение_порта() -> tuple[list[str], int]:
    """
    Занятый порт и порт без прав — разные новости, и путать их нельзя.

    Первое, во что упирается проверяющий, — сообщение о порте. Ветка
    «уже слушают» видена живьём (журнал, 23 сентября), а ветка «нет
    прав» на этой машине не срабатывает: порт 80 здесь отдаётся
    обычному пользователю. Она была написана и ни разу не проверена.

    Поэтому построенный случай: `объяснить_порт` зовётся с подставленным
    `errno`. Требуется, чтобы каждая ветка сказала своё и не сказала
    чужого — иначе «нет прав» однажды посоветует `lsof`, а занятый порт
    предложит взять номер побольше, и оба совета будут мимо.

    Второе возвращаемое — сколько случаев разобрано.
    """
    import errno as _e
    from .server import объяснить_порт

    случаи = (
        (_e.EADDRINUSE, "занят",   ("уже кто-то слушает", "lsof", "8001"),
                                   ("без особых прав",)),
        (_e.EACCES,     "нет прав", ("без особых прав", "8000"),
                                    ("уже кто-то слушает", "lsof")),
        (_e.EPERM,      "нет прав", ("без особых прав", "8000"),
                                    ("уже кто-то слушает", "lsof")),
        (_e.ENOTSOCK,   "прочее",  ("открыть не вышло",),
                                   ("уже кто-то слушает", "без особых прав")),
    )
    плохо: list[str] = []
    for код, имя, обязано, запрещено in случаи:
        текст = "\n".join(объяснить_порт(8000, "восток", OSError(код, "проба")))
        for кусок in обязано:
            if кусок not in текст:
                плохо.append(f"{имя} ({_e.errorcode[код]}): нет «{кусок}»")
        for кусок in запрещено:
            if кусок in текст:
                плохо.append(f"{имя} ({_e.errorcode[код]}): лишнее «{кусок}»")
    return плохо, len(случаи)


def _команды_для_обеих_систем(корень: Path | None = None) -> tuple[list[str], int]:
    """
    Документ, советующий `python3` или `zsh`, обязан сказать и про Windows.

    Первое, что делает проверяющий, — выполняет команду из README. На
    Windows команды `python3` **нет**: в Windows 10 и 11 включён
    системный псевдоним `python3.exe`, который открывает Microsoft Store.
    То есть первая же строка нашего README либо не выполнится, либо
    откроет магазин приложений. `zsh` там тоже нет, а
    `проверка-с-нуля.sh` мы сами советуем как «то, что увидите вы».

    Сторож простой: в документе, где встречается `python3 -m` или
    `zsh …`, обязано найтись и упоминание пути для Windows — `py -3` или
    один из наших `.bat`. Второе возвращаемое — сколько документов
    сверено; ноль означает, что сверять было нечего, и это тоже красный:
    команды могли переписать так, что сторож их больше не видит.
    """
    import re
    корень = корень or Path(__file__).resolve().parents[1]
    юниксовая = re.compile(r"python3\s+-m|\bzsh\s+\S+\.sh")
    виндовая = re.compile(r"py\s+-3|\.bat\b")
    плохо: list[str] = []
    сверено = 0
    for имя in _ДОКУМЕНТЫ_НАРУЖУ:
        файл = корень / имя
        if not файл.exists():
            continue
        текст = файл.read_text(encoding="utf-8")
        if not юниксовая.search(текст):
            continue
        сверено += 1
        if not виндовая.search(текст):
            плохо.append(f"{имя}: советует python3/zsh, а про Windows молчит "
                         f"(там нет ни того, ни другого)")
    return плохо, сверено


# Где документы обещают, сколько отметок даёт проверка. Файл, которого
# нет, пропускается: `CLAUDE.md` в публичный снимок не едет, а `README.md`
# едет всегда.
_ГДЕ_ОТМЕТКИ = ("README.md", "CLAUDE.md")


def _отметки_в_документах(команда: str, сколько: int,
                          корень: Path | None = None) -> tuple[list[str], int]:
    """
    Документы называют число отметок — оно обязано совпадать с настоящим.

    Число отметок пересчитывали руками и руками же забывали: 23 сентября
    в README стояло 48, в `CLAUDE.md` 44, а проверка давала 55. Читателю
    это говорит «здесь считали давно», и он прав.

    Сторож ищет строку запуска (`python3 -m vrptw.selfcheck` или
    `… server check`) и следующее за ней число перед словом «отмет».
    Второе возвращаемое — сколько мест сверено; ноль означает, что
    проверять было нечего, и это тоже повод покраснеть: строку могли
    переписать так, что сторож её больше не видит.
    """
    import re
    корень = корень or Path(__file__).resolve().parents[1]
    образец = re.compile(re.escape(команда) + r"[^\n]*?(\d+)\s+отмет")
    плохо: list[str] = []
    сверено = 0
    for имя in _ГДЕ_ОТМЕТКИ:
        файл = корень / имя
        if not файл.exists():
            continue
        for номер, строка in enumerate(файл.read_text(encoding="utf-8").split("\n"), 1):
            совпало = образец.search(строка)
            if not совпало:
                continue
            сверено += 1
            обещано = int(совпало.group(1))
            if обещано != сколько:
                плохо.append(f"{имя}:{номер} обещает {обещано} отметок, "
                             f"а «{команда}» даёт {сколько}")
    return плохо, сверено


def _имена_не_повторяются(дни: dict) -> tuple[list[str], int]:
    """У сорока двух человек — сорок два имени, а не четырнадцать на всех.

    Инженеры зон — наше допущение, но людей в нём сорок два: по
    четырнадцать на участок, каждый со своим офисом и своим днём. В одном
    плане двое из разных зон не встречаются никогда, поэтому солверу
    совпадение имён безразлично — а интерфейс собирает базу инженеров из
    всех расчётов сразу, и до 25 сентября она показывала сорок две записи
    на четырнадцать имён. Выглядело это как один человек на трёх участках
    со сменами внахлёст; метка «в трёх участках, смены пересекаются» у
    интерфейса загоралась на всех четырнадцати разом.

    Причина была в одной строке: `ИМЕНА[i % len(ИМЕНА)]`, а `i` в каждой
    зоне шёл от нуля. Теперь у зоны свой блок имён, и ключ блока — адрес
    офиса (`_блок_офиса`), чтобы выгрузка `восток.csv` дала тех же людей,
    что зона «восток».

    Второе число — сколько имён сверено; ноль означал бы, что сверять
    было нечего.
    """
    все: list[str] = []
    где: dict[str, list[str]] = {}
    for з, д in дни.items():
        for e in д.engineers:
            все.append(e.name)
            где.setdefault(e.name, []).append(з)
    повторы = [f"«{имя}» в зонах {', '.join(зоны)}"
               for имя, зоны in sorted(где.items()) if len(зоны) > 1]
    return повторы, len(все)


def _одна_точка_выезда(дни: dict) -> tuple[list[str], int]:
    """
    Вся бригада зоны выезжает из офиса зоны — и ниоткуда больше.

    Постановщику задали этот вопрос прямо (стенограмма 16 сентября,
    00:07:25 — «верно ли брать адрес офиса в последней строке файла как
    старт для всех инженеров региона?»), и ответ был прямой: у каждого
    региона свой офис и своя точка старта. Одна.

    До 24 сентября модуль загрузки сажал одного человека в дальнем
    городе, если там набиралась группа заявок: на юго-востоке выходило
    две точки выезда вместо одной. Разрешения на это не было — цитата,
    которой оно оправдывалось, не нашлась ни в стенограмме, ни в отчёте,
    ни в переписке, — и вдобавок `ДОПУЩЕНИЯ` всё это время говорили, что
    стартовая точка берётся из выгрузки как есть.

    Адрес офиса сторож берёт **из самого файла зоны**, а не из дня:
    иначе проверка «все в одной точке» прошла бы зелёной и для бригады,
    целиком переехавшей в Каширу. Второе число — сколько инженеров
    сверено; ноль означал бы, что проверять было нечего.
    """
    from .core import geocode
    from .core.load import REGIONS, read_orders_csv
    плохо: list[str] = []
    сверено = 0
    for з, д in дни.items():
        _, адрес_офиса = read_orders_csv(REGIONS[з])
        точка = geocode.known([адрес_офиса]).get(адрес_офиса) or {}
        if not точка.get("lat"):
            плохо.append(f"{з}: офис «{адрес_офиса}» не геокодирован")
            continue
        офис = (round(точка["lat"], 6), round(точка["lon"], 6))
        узел = д.network.nearest_node(точка["lat"], точка["lon"])
        for e in д.engineers:
            сверено += 1
            свой = (round(e.home_lat, 6), round(e.home_lon, 6))
            if свой != офис:
                плохо.append(f"{з}: {e.id} {e.name} выезжает не из офиса "
                             f"({e.home_address}), а из {свой} вместо {офис}")
            elif e.home_node != узел:
                плохо.append(f"{з}: {e.id} стоит в узле {e.home_node}, "
                             f"а офис — узел {узел}")
    return плохо, сверено


def _без_кодировки(корень: Path | None = None) -> list[str]:
    """Текстовый ввод-вывод движка без явной кодировки: «файл:строка».

    На Windows кодировка по умолчанию — cp1251, и `read_text()` без
    `encoding` ломался на кириллице готовых планов и сетей. Ищем по дереву
    разбора, а не по тексту: вызов на три строки текстовый поиск не видит.

    Что считается текстом — `_ТЕКСТОВЫЕ`, `_ОТКРЫТИЯ` и `_ПОДПРОЦЕСС`
    рядом. Кодировка засчитывается и по имени, и позицией: `open(p, "w",
    -1, "utf-8")` — это явная кодировка, а не дыра.

    Чего сторож не видит и видеть не берётся: режим в переменной (такой
    вызов считается текстовым — для сторожа это безопасная сторона),
    обёртки своего сочинения и `TextIOWrapper` поверх двоичного потока.

    `корень` — где искать; по умолчанию пакет движка. Параметр нужен
    самопроверке: `_ПРОБА_КОДИРОВОК` ниже — это набор вызовов с заранее
    известным ответом, и проверка требует, чтобы сторож нашёл ровно
    помеченные строки, не больше и не меньше.
    """
    import ast

    def ключ(вызов, имя):
        return next((k for k in вызов.keywords if k.arg == имя), None)

    def места(вызов):
        """Чем вызов является, если текстовым. `None` — не наше дело."""
        ф = вызов.func
        имя = ф.attr if isinstance(ф, ast.Attribute) else getattr(ф, "id", "")
        основа = getattr(ф.value, "id", "") if isinstance(ф, ast.Attribute) else ""
        if имя == "open":
            if основа == "webbrowser":
                return None
            if isinstance(ф, ast.Name):
                return _ОТКРЫТИЯ[""]
            # `модуль.open` — по модулю; всё прочее (`путь.open(...)`,
            # `self.файл.open(...)`) считаем `Path.open`: режим первый.
            return _ОТКРЫТИЯ.get(основа, (0, 2, "r"))
        return _ТЕКСТОВЫЕ.get(имя)

    def текстом(вызов, где, по_умолчанию):
        режим = ключ(вызов, "mode")
        узел = режим.value if режим else (
            вызов.args[где] if где is not None and len(вызов.args) > где else None)
        if узел is None:
            return "b" not in по_умолчанию
        return not (isinstance(узел, ast.Constant) and isinstance(узел.value, str)
                    and "b" in узел.value)

    корень = корень or Path(__file__).resolve().parent
    плохие = []
    for путь in sorted(корень.rglob("*.py")):
        if "__pycache__" in путь.parts:
            continue
        for узел in ast.walk(ast.parse(путь.read_text(encoding="utf-8"))):
            if not isinstance(узел, ast.Call) or ключ(узел, "encoding"):
                continue
            ф = узел.func
            имя = ф.attr if isinstance(ф, ast.Attribute) else getattr(ф, "id", "")
            если_плохо = f"{путь.relative_to(корень.parent)}:{узел.lineno}"
            if имя in _ПОДПРОЦЕСС:
                if any((k.arg in ("text", "universal_newlines")
                        and getattr(k.value, "value", None) is True)
                       or k.arg == "errors" for k in узел.keywords):
                    плохие.append(если_плохо)
                continue
            место = места(узел)
            if место is None:
                continue
            и_режим, и_кодировка, по_умолчанию = место
            # Кодировку могли передать позицией, а не по имени.
            if и_кодировка is not None and len(узел.args) > и_кодировка:
                continue
            if текстом(узел, и_режим, по_умолчанию):
                плохие.append(если_плохо)
    return плохие


def _замок_не_виснет() -> list[str]:
    """Windows-замок не должен превращаться в вечный круг.

    Две подмены `msvcrt`. Первая отказывает мгновенно — так ведёт себя
    `LK_LOCK` на закрытом дескрипторе, в сетевой папке, на плейсхолдере
    OneDrive, на файловой системе без блокировок; от неё `запереть`
    обязан отступиться сразу и вернуть «замка нет». Вторая честно стоит в
    очереди и со второго раза пускает; от неё — дождаться замка.

    Прежний код на первой подмене крутился вечно и съедал ядро, поэтому
    зовём в отдельном потоке и со сроком: проверка, которая сама виснет,
    хуже дефекта, который она ищет.
    """
    import io
    import threading
    import types
    from contextlib import redirect_stderr

    from . import compat

    def позвать(locking, срок: float):
        файл = types.SimpleNamespace(seek=lambda _: None, fileno=lambda: 3)
        итог: dict = {}

        def работа():
            try:
                итог["взят"] = compat.запереть(файл)
            except BaseException as e:      # падение — тоже ответ
                итог["упал"] = f"{type(e).__name__}: {e}"

        было = compat.fcntl, compat.msvcrt, compat._сказано_про_замок
        compat.fcntl = None
        compat.msvcrt = types.SimpleNamespace(LK_LOCK=1, LK_UNLCK=0, locking=locking)
        compat._сказано_про_замок = False
        поток = threading.Thread(target=работа, daemon=True)
        t = time.time()
        # Подмена говорит в stderr «пишу без замка» — это её работа, но в
        # выводе проверки такой строке делать нечего.
        with redirect_stderr(io.StringIO()):
            поток.start()
            поток.join(срок)
        повис = поток.is_alive()
        compat.fcntl, compat.msvcrt, compat._сказано_про_замок = было
        return итог, повис, time.time() - t

    плохо: list[str] = []

    мгновенных: list = []

    def мгновенно(fd, режим, сколько):
        мгновенных.append(режим)
        raise OSError(9, "Bad file descriptor")

    итог, повис, сколько_ждали = позвать(мгновенно, 5.0)
    if повис:
        плохо.append(f"мгновенный отказ: `запереть` не вернулась за 5 с, "
                     f"попыток уже {len(мгновенных)}")
    elif итог.get("упал"):
        плохо.append(f"мгновенный отказ: {итог['упал']}")
    elif итог.get("взят") is not False:
        плохо.append(f"мгновенный отказ: вернула {итог.get('взят')!r}, "
                     f"а замка нет")
    elif len(мгновенных) > 50:
        плохо.append(f"мгновенный отказ: {len(мгновенных)} попыток за "
                     f"{сколько_ждали:.1f} с — это круг, а не отступление")

    очередных: list = []

    def очередь(fd, режим, сколько):
        очередных.append(режим)
        if len(очередных) == 1:
            # Так ведёт себя занятый файл: `LK_LOCK` отстоял очередь и
            # только потом отказал.
            time.sleep(compat.ЗАМОК_ОЧЕРЕДЬ + 0.05)
            raise OSError(36, "Resource deadlock avoided")

    итог, повис, _ = позвать(очередь, compat.ЗАМОК_ОЧЕРЕДЬ + 5.0)
    if повис or итог.get("упал"):
        плохо.append(f"занятый файл: {итог.get('упал') or 'не дождалась замка'}")
    elif итог.get("взят") is not True:
        плохо.append(f"занятый файл: вернула {итог.get('взят')!r} вместо замка "
                     f"после {len(очередных)} попыток")
    elif len(очередных) < 2:
        плохо.append(f"занятый файл: попыток {len(очередных)} — замок взят "
                     f"без ожидания, подмена не сработала")

    return плохо


# Проба для сторожа кодировок: вызовы с заранее известным ответом.
# Помеченные «ПЛОХО» он обязан найти все, помеченные «ХОРОШО» — ни одного.
# Без такой пробы «сторож ничего не нашёл» ничего не значит: сломанный до
# немоты сторож выглядит ровно так же, как чистый движок. Файл только
# разбирается, не исполняется.
_ПРОБА_КОДИРОВОК = """\
import gzip, io, os, subprocess, tempfile, webbrowser
from pathlib import Path

p, fd, cmd, s, url = Path("x"), 0, ["ls"], "текст", "http://x"


def случаи():
    open(p)                                             # ПЛОХО
    open(p, "w")                                        # ПЛОХО
    open(p, "w", encoding="utf-8")                      # ХОРОШО
    open(p, "rb")                                       # ХОРОШО
    open(p, mode="w")                                   # ПЛОХО
    open(p, mode="rb")                                  # ХОРОШО
    open(p, "w", -1, "utf-8")                           # ХОРОШО
    p.open()                                            # ПЛОХО
    p.open("w")                                         # ПЛОХО
    p.open("rb")                                        # ХОРОШО
    p.open("w", encoding="utf-8")                       # ХОРОШО
    p.open("w", -1, "utf-8")                            # ХОРОШО
    io.open(p, "w")                                     # ПЛОХО
    io.open(p, "rb")                                    # ХОРОШО
    io.open(p, "w", encoding="utf-8")                   # ХОРОШО
    os.fdopen(fd, "w")                                  # ПЛОХО
    os.fdopen(fd, "wb")                                 # ХОРОШО
    gzip.open(p)                                        # ХОРОШО
    gzip.open(p, "rb")                                  # ХОРОШО
    gzip.open(p, "rt")                                  # ПЛОХО
    gzip.open(p, "rt", encoding="utf-8")                # ХОРОШО
    p.read_text()                                       # ПЛОХО
    p.read_text(encoding="utf-8")                       # ХОРОШО
    p.read_text("utf-8")                                # ХОРОШО
    p.write_text(s)                                     # ПЛОХО
    p.write_text(s, encoding="utf-8")                   # ХОРОШО
    p.write_text(s, "utf-8")                            # ХОРОШО
    tempfile.NamedTemporaryFile()                       # ХОРОШО
    tempfile.NamedTemporaryFile("w")                    # ПЛОХО
    tempfile.NamedTemporaryFile("w", encoding="utf-8")  # ХОРОШО
    tempfile.NamedTemporaryFile("wb")                   # ХОРОШО
    tempfile.NamedTemporaryFile("w", -1, "utf-8")       # ХОРОШО
    tempfile.TemporaryFile("w")                         # ПЛОХО
    subprocess.run(cmd, text=True)                      # ПЛОХО
    subprocess.run(cmd, text=True, encoding="utf-8")    # ХОРОШО
    subprocess.run(cmd, errors="replace")               # ПЛОХО
    subprocess.run(cmd, encoding="utf-8")               # ХОРОШО
    subprocess.Popen(cmd, universal_newlines=True)      # ПЛОХО
    subprocess.run(cmd)                                 # ХОРОШО
    webbrowser.open(url)                                # ХОРОШО
"""


def _сторож_на_пробе() -> tuple[list[int], list[int], int, int]:
    """Прогнать сторож кодировок по пробе.

    Возвращает: что пропустил, на что отругался зря, сколько всего надо
    было поймать и сколько пропустить. Две первые пустые — сторож цел.
    """
    import shutil
    import tempfile

    каталог = Path(tempfile.mkdtemp(prefix="vrptw-проба-кодировок-"))
    try:
        (каталог / "случаи.py").write_text(_ПРОБА_КОДИРОВОК, encoding="utf-8")
        строки = _ПРОБА_КОДИРОВОК.splitlines()
        ждём = {i for i, с in enumerate(строки, 1) if с.rstrip().endswith("# ПЛОХО")}
        можно = {i for i, с in enumerate(строки, 1) if с.rstrip().endswith("# ХОРОШО")}
        нашлось = {int(x.rsplit(":", 1)[1]) for x in _без_кодировки(каталог)}
        return sorted(ждём - нашлось), sorted(нашлось - ждём), len(ждём), len(можно)
    finally:
        shutil.rmtree(каталог, ignore_errors=True)


# Что делает дочерний процесс проверки «как на Windows»: в кодировке cp1251
# и без `fcntl` загружает восток (сети с кириллицей), пишет архив под
# замком, читает его обратно и печатает «✓» в канал. `msvcrt` подменён
# записывающим: на Mac его нет, а иначе ветку замка для Windows не
# проверяло бы ничто. Вывод не падает сам — `vrptw/__init__.py`.
_КАК_НА_WINDOWS = """
import sys, locale, tempfile, types
sys.modules["fcntl"] = None
import vrptw.compat as compat
# msvcrt подменяем полем модуля, а не в sys.modules: по нему стандартная
# библиотека решает, что это Windows, и лезет за `_winapi`.
вызовы = []
compat.msvcrt = types.SimpleNamespace(
    LK_LOCK=1, LK_UNLCK=0,
    locking=lambda fd, режим, сколько: вызовы.append((режим, сколько)))
from vrptw.core.load import load_day
from vrptw.state import use_state_dir, runs_path, изменить_список, прочитать_список
день = load_day("восток", quiet=True)
# cp1251 раскодирует почти любые байты: не упасть мало, район из файла
# сети должен прочитаться тем же словом, а не кракозябрами.
assert "Нижегородский" in {a.district for a in день.network.nodes}, "районы сети — не той кодировкой"
use_state_dir(tempfile.mkdtemp(prefix="vrptw-check-cp1251-"))
изменить_список(runs_path(), lambda список: список.append({"note": "проверка ✓ кириллицей"}))
assert прочитать_список(runs_path())[0]["note"] == "проверка ✓ кириллицей"
assert вызовы == [(1, 1), (0, 1)], f"замок msvcrt: {вызовы}"
print("✓", len(день.orders), locale.getpreferredencoding(False))
"""


def main() -> int:
    global отметок
    failures.clear()
    отметок = 0

    t_all = time.time()
    данные_до = _отпечаток_данных()

    # Правило 9: проверка работает не в боевом состоянии. Каталог
    # уводится в сторону здесь, до всего остального, а отпечаток боевого
    # снимается до увода — чтобы было с чем сверить в конце.
    from .state import state_dir, use_state_dir
    боевой_каталог = state_dir().resolve()
    состояние_до = _боевое_состояние(боевой_каталог)
    use_state_dir(tempfile.mkdtemp(prefix="vrptw-selfcheck-"))

    # --- 1. окружение ---
    step("1. Окружение")
    if sys.version_info < (3, 10):
        bad(f"нужен Python 3.10 или новее, у вас {sys.version.split()[0]}")
        return 1
    ok(f"Python {sys.version.split()[0]}")

    try:
        import numpy
        import scipy
        ok(f"numpy {numpy.__version__}, scipy {scipy.__version__}")
    except ImportError as e:
        bad(f"не хватает библиотеки: {e.name}. Поставьте: pip install numpy scipy")
        return 1

    # Windows у проверяющих: `fcntl` там нет, кодировка по умолчанию —
    # cp1251. Первое роняло импорт сервера, второе — чтение готовых планов
    # и сетей (compat.py). Статически — явная кодировка везде; вживую —
    # дочерний процесс в cp1251 и без `fcntl`.
    # Сперва сам сторож — на пробе с заранее известным ответом.
    пропущено, лишнее, ловить, пропускать = _сторож_на_пробе()
    if пропущено or лишнее:
        bad(f"сторож кодировок неисправен: пропускает строки {пропущено}, "
            f"ругается зря на {лишнее} (проба — `_ПРОБА_КОДИРОВОК`)")
    else:
        ok(f"сторож кодировок проверен на пробе: ловит все {ловить} плохих "
           f"вызова, молчит на всех {пропускать} хороших")

    мимо, посторонних = _контрольные_не_читаются()
    if мимо:
        bad(f"контрольные выгрузки: {'; '.join(мимо[:2])}")
    elif посторонних:
        ok(f"контрольные выгрузки не читает ни один путь: загрузчик знает три "
           f"файла, каталог не обходится маской, {посторонних} посторонних не "
           f"упомянуты нигде")
    else:
        ok("контрольных выгрузок нет вовсе: загрузчик знает три файла "
           "заявок, каталог не обходится маской — как и должно быть в снимке")

    обрывы = _ссылки_наружу()
    if обрывы:
        bad(f"документы наружу ссылаются на то, чего в снимке нет: "
            f"{', '.join(обрывы[:3])}{' …' if len(обрывы) > 3 else ''}")
    else:
        ok(f"ссылки документов наружу ведут в снимок или честно говорят, "
           f"что файл в него не входит")

    мимо_порога, порог = _порог_приоритета()
    if мимо_порога:
        bad(f"приоритет в документах обещает больше, чем дают веса: "
            f"{'; '.join(мимо_порога[:2])}")
    else:
        ok(f"приоритет: документы называют тот же порог, что дают веса — "
           f"авария дороже {порог} подключений, но не ста")

    чужие = _чьи_данные()
    if чужие:
        bad(f"данные названы заказчиковыми: {', '.join(чужие[:3])}"
            f"{' …' if len(чужие) > 3 else ''} — выгрузка обезличенная и "
            f"выдана организаторами (правило 12)")
    else:
        ok(f"данные всюду названы выгрузкой организаторов: "
           f"{len(_ДОКУМЕНТЫ_НАРУЖУ)} документа наружу, сводки и выводы "
           f"замеров, и всё, что движок говорит вслух")

    без = _без_кодировки()
    if без:
        bad(f"текстовый ввод-вывод без encoding: {', '.join(без[:4])}"
            f"{' …' if len(без) > 4 else ''} — на Windows это cp1251")
    else:
        ok("чтение и запись текста в движке — с явной кодировкой")
    import os as _os
    import subprocess as _sp
    среда = {**_os.environ, "LC_ALL": "ru_RU.CP1251", "PYTHONUTF8": "0",
             "PYTHONPATH": str(Path(__file__).resolve().parent.parent)}
    среда.pop("PYTHONIOENCODING", None)
    # Скрипт — через stdin, а не `-c`: на Linux аргументы командной строки
    # расшифровываются по локали, и кириллица в cp1251 ломала сам скрипт.
    дочерний = _sp.run([sys.executable, "-"], input=_КАК_НА_WINDOWS.encode("utf-8"),
                       env=среда, capture_output=True, timeout=120)
    ответ = дочерний.stdout.decode("cp1251", "replace").strip()
    if дочерний.returncode == 0 and ответ.startswith(("✓", "?")):
        кодировка = ответ.split()[-1]
        ok(f"как на Windows: без fcntl (замок — msvcrt, подменённый), {кодировка} — "
           f"день, архив и вывод целы"
           + ("" if кодировка.lower().replace("-", "") == "cp1251"
              else " (локали cp1251 здесь нет — проверено только без fcntl)"))
    else:
        try:                            # дочерний пишет в своей кодировке
            ошибка = дочерний.stderr.decode("utf-8")
        except UnicodeDecodeError:
            ошибка = дочерний.stderr.decode("cp1251", "replace")
        хвост = ошибка.strip().splitlines()[-1:]
        bad(f"как на Windows (без fcntl, cp1251) не работает: {хвост}")

    висит = _замок_не_виснет()
    if висит:
        bad(f"замок на Windows: {'; '.join(висит)}")
    else:
        ok("замок на Windows: занятый файл ждёт, на отказ без очереди "
           "отступается и пишет без замка — не крутится")

    # --- 2. дорожная сеть ---
    step("2. Дорожная сеть Москвы")
    t = time.time()
    from .core.network import default_network
    net = default_network()
    ok(f"{len(net.nodes)} узлов, матрицы времени по 14 часам — {time.time() - t:.1f} с")

    mid_day = net.travel_minutes(0, 200, 13 * 60)
    rush = net.travel_minutes(0, 200, 18 * 60)
    if rush > mid_day:
        ok(f"час пик учитывается: та же поездка 13:00 — {mid_day:.0f} мин, 18:00 — {rush:.0f} мин")
    else:
        bad(f"час пик не работает: 13:00 {mid_day:.0f} мин, 18:00 {rush:.0f} мин")

    # Без файлов карты — отказ, а не молчаливая синтетика. Прежде сеть
    # откатывалась на выдуманную одной строкой в выводе, и все числа
    # дальше были бы сняты на другой карте.
    import os as _os
    from .core import osm as _osm
    _было_адреса, _было_синт = _osm.ADDRESS_FILE, _os.environ.pop("VRPTW_SYNTHETIC_MAP", None)
    _osm.ADDRESS_FILE = _osm.DATA_DIR / "нет-такого-файла.json"
    try:
        _без_данных = type(default_network()).__name__
    except FileNotFoundError:
        _без_данных = None
    finally:
        _osm.ADDRESS_FILE = _было_адреса
        if _было_синт is not None:
            _os.environ["VRPTW_SYNTHETIC_MAP"] = _было_синт
    if _без_данных is None:
        ok("без файлов карты — отказ с объяснением, а не синтетика молча")
    else:
        bad(f"без файлов карты стенд молча взял {_без_данных}")

    # Без интернета недостающий перегон рисуется прямой сразу и в кэш не
    # ложится. Прежде он стоил пятнадцать секунд — два захода с паузами —
    # и прямая оставалась в кэше навсегда.
    if hasattr(net, "_geometry"):
        _было_osrm, _было_офлайн = _osm.OSRM, _osm._offline_until
        _осрм_env = _os.environ.pop("VRPTW_OFFLINE", None)
        _osm.OSRM, _osm._offline_until = "http://127.0.0.1:9", 0.0
        пары = [(a, b) for a in range(40) for b in range(40)
                if a != b and not net.has_geometry(a, b)][:3]
        t0 = time.time()
        линии = [net.path_latlon(a, b, 12 * 60) for a, b in пары]
        сек = time.time() - t0
        _osm.OSRM, _osm._offline_until = _было_osrm, _было_офлайн
        if _осрм_env is not None:
            _os.environ["VRPTW_OFFLINE"] = _осрм_env
        застряли = [f"{a}-{b}" for a, b in пары if net.has_geometry(a, b)]
        if сек < 8 and all(len(x) == 2 for x in линии) and not застряли:
            ok(f"без сети {len(пары)} недостающих перегона — прямые за {сек:.1f} с, "
               f"в кэш не легли")
        else:
            bad(f"без сети {len(пары)} перегона заняли {сек:.1f} с, в кэше "
                f"осели {застряли}")

    # Кэш ломаных у каждой сети свой, и каждая ломаная ведёт от своего
    # дома к своему. Номера узлов у сетей свои: прежде сети зон читали
    # московский кэш, и 47 его перегонов попадали в номера югоцентра —
    # чужая ломаная начиналась в 13,7 км от точки выезда. У зон кэш полный:
    # на защите ни один перегон не должен идти в сеть.
    from .core.geo import haversine_km as _km
    from .core.load import REGIONS as _ЗОНЫ, load_day as _load_day

    def _кривые(сеть) -> list[str]:
        плохо = []
        for ключ, линия in сеть._geometry.items():
            номера = [int(x) for x in ключ.split("-")]
            if max(номера) >= len(сеть.nodes):      # ключ чужой сети
                плохо.append(ключ)
                continue
            a, b = (сеть.nodes[i] for i in номера)
            if (_km(a.lat, a.lon, линия[0][0], линия[0][1]) > 0.5
                    or _km(b.lat, b.lon, линия[-1][0], линия[-1][1]) > 0.5):
                плохо.append(ключ)
        return плохо

    дни_зон = {з: _load_day(з, quiet=True) for з in _ЗОНЫ}
    сети = [("Москва", net)] + [(з, д.network) for з, д in дни_зон.items()]
    кривых = {имя: _кривые(с) for имя, с in сети if hasattr(с, "_geometry")}
    неполные = {имя: f"{len(с._geometry)} из {len(с.nodes) * (len(с.nodes) - 1)}"
                for имя, с in сети[1:]
                if len(с._geometry) < len(с.nodes) * (len(с.nodes) - 1)}
    общий = [имя for имя, с in сети[1:] if с._geometry_file == net._geometry_file]
    if any(кривых.values()) or неполные or общий:
        bad(f"ломаные: чужие {({k: len(v) for k, v in кривых.items() if v})}, "
            f"неполные {неполные}, общий с Москвой кэш {общий}")
    else:
        ok(f"ломаные своих сетей ведут от дома к дому; у зон все "
           f"{sum(len(с._geometry) for _, с in сети[1:])} пар в кэше")

    разброс, привязка = _аварии_на_зонах(дни_зон)
    if разброс:
        bad(f"аварии на зонах падают в немногие дома или не в дома заявок: {разброс}")
    else:
        ok("аварии на зонах — по домам заявок зоны, с их адресами, не кучей")
    if привязка:
        bad(f"авария «у кого на участке» не у его точки: {привязка}")
    else:
        ok("авария «у кого на участке» — в восьми ближайших к инженеру домах")

    повторы_имён, сверено_имён = _имена_не_повторяются(дни_зон)
    if повторы_имён or not сверено_имён:
        bad(f"имена инженеров повторяются между зонами: {повторы_имён[:2]}, "
            f"сверено имён {сверено_имён}")
    else:
        ok(f"у каждой зоны свои имена: {сверено_имён} человек, "
           f"{сверено_имён} разных имён")

    не_из_офиса, сверено_домов = _одна_точка_выезда(дни_зон)
    if не_из_офиса or not сверено_домов:
        bad(f"стартовая точка не одна на зону: {не_из_офиса[:2]}, "
            f"сверено инженеров {сверено_домов}")
    else:
        ok(f"вся бригада выезжает из офиса зоны — одна точка, как ответил "
           f"постановщик (сверено инженеров {сверено_домов})")

    врут, недосягаемых = _досягаемость_в_причинах(дни_зон)
    if врут:
        bad(f"причина неназначенной заявки расходится с дорогой: {врут[:2]}")
    else:
        ok(f"причина неназначенной заявки не спорит с дорогой на всех зонах "
           f"(недосягаемых на живых данных {недосягаемых})")

    кривая_проба = _ПРОБА_ДОСЯГАЕМОСТИ(дни_зон["юго-восток"])
    if кривая_проба:
        bad(f"проба досягаемости: {кривая_проба}")
    else:
        ok("недосягаемость отличима от занятости — три построенных случая")

    # «г.Город Москва» — только показ. Строка выгрузки остаётся ключом
    # кэша адресов: заявка с таким адресом обязана сохранить координату.
    from .core.load import read_orders_csv as _строки
    дубль, потеряны = [], []
    for з, д in дни_зон.items():
        заявки = {o.id: o for o in д.orders}
        дубль += [o.id for o in д.orders if (o.address or "").lower().startswith("г.город")]
        потеряны += [r["id"] for r in _строки(_ЗОНЫ[з])[0]
                     if (r["address"] or "").lower().startswith("г.город")
                     and not (r["id"] in заявки and заявки[r["id"]].lat)]
    if дубль or потеряны:
        bad(f"адреса: «г.Город» на показе у {дубль}, без координаты {потеряны}")
    else:
        ok("адреса на показ без «г.Город», координаты по строке выгрузки")

    # --- 3. день ---
    step("3. Синтетический день")
    from .core.generate import day_stats, generate_day
    day = generate_day(seed=1, network=net)
    st = day_stats(day)
    ok(f"{st['orders']} заявок, {st['engineers']} инженеров, "
       f"{st['est_work_hours']} ч работы на {st['capacity_hours']} ч смен")
    ok(f"навыки — заявок {st['orders_by_skill']}, инженеров {st['engineers_by_skill']}")

    карта = "москва" if getattr(net, "has_addresses", False) else "синтетика"
    отпечаток = _поток_аварий(net)
    if отпечаток == ПОТОК_АВАРИЙ[карта]:
        ok(f"поток аварий на {карта == 'москва' and 'московском пуле' or 'синтетике'} прежний: {отпечаток}")
    else:
        bad(f"поток аварий ({карта}) сдвинулся: {отпечаток} вместо "
            f"{ПОТОК_АВАРИЙ[карта]} — замеры пересчёта на синтетике не сравнимы")

    # Пятнадцатого сентября генератор заменится на загрузчик настоящих
    # заявок, и первое, что понадобится, — ответ «годен ли этот день».
    from .core.validate_day import check as check_day

    broken, warned = check_day(day)
    if broken:
        bad(f"синтетический день не проходит собственную проверку: {broken[:2]}")
    else:
        ok(f"день годен к планированию, предупреждений {len(warned)}")

    # Проверяльщик, который ничего не ловит, хуже отсутствующего.
    import copy as _copy

    probe = _copy.copy(day)
    probe.orders = [_copy.copy(o) for o in day.orders]
    probe.orders[0].est_minutes = 0
    probe.engineers = [_copy.copy(e) for e in day.engineers]
    probe.engineers[0].shift_end = 23 * 60
    caught, _ = check_day(probe)
    if len(caught) < 2:
        bad(f"проверка дня не заметила ни нулевой длительности, ни смены "
            f"за 21:00: нашла {caught}")
    else:
        ok("проверка дня ловит порчу: нулевая длительность и смена за 21:00")

    # Инвариант плана — тоже проверяльщик, и к нему то же требование.
    # Самое крупное слагаемое времени — длительность работ — не
    # проверялось до 20 сентября вовсе: план, где каждый визит длится
    # минуту вместо часа, проходил чисто, если согласованно пересчитать
    # дорогу. А `duration_factor` это ручка пользователя от 0,5 до 2,0.
    #
    # Подделка строится ровно так, как её построил бы желающий показать
    # красивое покрытие: дорога честная, окна соблюдены, сокращена только
    # работа. Прежде такой план не отличался от настоящего ничем.
    from .solvers import fast as _fast
    from .solvers import improved as _improved

    честный = _fast.solve(day, params=_improved.Params(lns_seconds=0.0,
                                                       lns_iterations=120))
    поддельный = _copy.deepcopy(честный)
    for _r in поддельный.routes.values():
        for _v in _r.visits:
            _v.finish = _v.start + 1          # «работа длится минуту»
    # Считаем именно жалобы на длительность: подделка задевает и дорогу
    # (следующий визит выезжает раньше), и такие жалобы к делу не
    # относятся — засчитать их значило бы отчитаться чужой работой.
    сокращено = sum(1 for p in поддельный.check_invariants(day)
                    if "по нормативу" in p)
    # И вторая половина: работа не может начаться раньше приезда.
    рано = _copy.deepcopy(честный)
    for _r in рано.routes.values():
        for _v in _r.visits:
            _v.start = _v.arrive - 15
            _v.finish = _v.start + _v.order.est_minutes
    до_приезда = sum(1 for p in рано.check_invariants(day) if "приехал" in p)

    if сокращено == 0 or до_приезда == 0:
        bad(f"инварианты не ловят подделку длительности: сокращённая работа "
            f"{сокращено} нарушений, работа до приезда {до_приезда}")
    else:
        ok(f"инварианты ловят подделку длительности: {сокращено} визитов "
           f"сокращено до минуты, {до_приезда} начато до приезда")

    # Инвариант судит по дню, а не по копиям, которые несёт план. План,
    # собранный на дне со сдвинутыми сменами или с облегчённой работой,
    # прежде сверялся сам с собой и проходил чисто.
    from .core.domain import InvalidPlan, Plan as _Plan
    from .shiftopt import apply as _apply_shifts

    _мало = _improved.Params(lns_seconds=0.0, lns_iterations=60)
    раньше = _apply_shifts(day, [e.shift_start // 60 - 1 for e in day.engineers],
                           shift_len=None)
    чужие_смены = _fast.solve(раньше, params=_мало).check_invariants(day)
    легче = _copy.copy(day)
    легче.orders = [_copy.copy(o) for o in day.orders]
    for _o in легче.orders:
        _o.est_minutes = max(1, _o.est_minutes // 2)
    чужая_работа = _fast.solve(легче, params=_мало).check_invariants(day)

    # Итог — множеством, а не счётом: одна заявка потеряна, другая дважды
    # в неназначенных, и по счёту всё сходилось.
    подмена = _copy.copy(честный)
    подмена.unassigned = честный.unassigned[1:] + [честный.unassigned[-1]]
    по_счёту = [p for p in подмена.check_invariants(day)
                if "потеряны" in p or "дважды" in p]
    if чужие_смены and чужая_работа and len(по_счёту) >= 2:
        ok(f"инвариант судит по дню: чужие смены — {len(чужие_смены)} нарушений, "
           f"облегчённая работа — {len(чужая_работа)}, потеря с дублем пойманы")
    else:
        bad(f"инвариант верит плану: чужие смены {len(чужие_смены)}, облегчённая "
            f"работа {len(чужая_работа)}, потеря с дублем {по_счёту}")

    # План проверяет себя сам, до того как уйти из солвера: подложенное
    # нарушение обязано остановить каждый из четырёх.
    from .solvers import greedy as _greedy
    from .solvers import sequential as _sequential
    _настоящая = _Plan.check_invariants
    _ноль = _improved.Params(lns_seconds=0.0, lns_iterations=0)
    _Plan.check_invariants = lambda self, d: ["подложено проверкой"]
    остановили = []
    try:
        for имя, решить in (("fast", lambda: _fast.solve(day, params=_ноль)),
                            ("improved", lambda: _improved.solve(day, params=_ноль)),
                            ("greedy", lambda: _greedy.solve(day)),
                            ("sequential", lambda: _sequential.solve(day))):
            try:
                решить()
            except InvalidPlan:
                остановили.append(имя)
    finally:
        _Plan.check_invariants = _настоящая

    # И голого `assert` на инвариантах нет нигде: под `python3 -O` он
    # исчезает, и проверка выключалась флагом интерпретатора.
    _голые = _голые_assert()
    if len(остановили) == 4 and not _голые:
        ok("каждый солвер проверяет свой план сам; голых assert на инвариантах "
           "нет во всём репозитории")
    else:
        bad(f"невалидный план выпускают: остановили только {остановили}; "
            f"голые assert: {_голые[:4]}")

    # «Сколько ещё людей нужно»: в голову ответа идёт НАИБОЛЬШЕЕ число
    # слотов среди прогонов, закрывших остаток, — надёжное, а не удачное.
    # Проверяется на выдуманных прогонах, а не на живых данных, нарочно:
    # на живых закрывшие прогоны совпадают по числу слотов, и замена
    # `max` на `min` там проходит зелёной — проверено.
    from .staffing import выбрать_голову

    def _прогон(открыто, осталось):
        return {"открыто": [None] * открыто, "не_закрыть": [None] * осталось}

    # Закрыли двое, +3 и +5: надёжное число — +5.
    расходятся = [_прогон(3, 0), _прогон(5, 0), _прогон(2, 1)]
    # Не закрыл никто: берём оставивший меньше всего, а из равных по
    # остатку — с меньшим числом слотов.
    никто = [_прогон(4, 2), _прогон(6, 1), _прогон(3, 1)]
    ok_надёжное = len(выбрать_голову(расходятся)["открыто"]) == 5
    г = выбрать_голову(никто)
    ok_лучший_из_худших = len(г["не_закрыть"]) == 1 and len(г["открыто"]) == 3
    from .staffing import описать_надёжность
    ok_разброс = ("от +3 до +5" in описать_надёжность(
        [_прогон(3, 0), _прогон(5, 0), _прогон(4, 0)]))
    ok_совпали = ("Одно и то же" in описать_надёжность(
        [_прогон(3, 0), _прогон(3, 0), _прогон(3, 0)]))
    if not (ok_разброс and ok_совпали):
        bad("«сколько людей нужно» неверно говорит о разбросе: при +3, +5 и "
            "+4 обязано назвать разброс, при трёх одинаковых — «одно и то же»")

    # Слова не должны спорить с кодом. До 23 сентября `staffing.method`
    # говорил «меньшее число слотов», а код брал наибольшее: поведение
    # сторож видел, описание метода — нет, и на экране диспетчера стояло
    # обратное тому, что метод делает. Теперь фраза одна, и она сверяется
    # с тем, что на самом деле выбрано.
    from .staffing import ГОЛОВА_СЛОВАМИ
    ok_слова = (("наибольш" in ГОЛОВА_СЛОВАМИ) == ok_надёжное)

    if ok_надёжное and ok_лучший_из_худших and ok_слова:
        ok("«сколько людей нужно» берёт надёжное число, а не лучшее зерно, "
           "и описывает это теми же словами")
    elif not ok_слова:
        bad(f"«сколько людей нужно»: код берёт "
            f"{'наибольшее' if ok_надёжное else 'не наибольшее'} число слотов, "
            f"а описание метода говорит «{ГОЛОВА_СЛОВАМИ}»")
    else:
        bad(f"«сколько людей нужно» выбирает не то: из закрывших +3 и +5 "
            f"взято +{len(выбрать_голову(расходятся)['открыто'])}, ожидалось "
            f"+5 — это лучшее зерно, выданное за вывод")

    leaked = [f for f in ("true_minutes", "true_no_show", "true_no_access")
              if hasattr(day.orders[0], f)]
    if leaked:
        bad(f"в заявке остались поля скрытой истины: {leaked} — планировщик может подглядеть")
    else:
        ok("скрытой истины в заявках нет: случайность разыгрывается в прогоне")

    # --- 4. планировщики ---
    step("4. Планировщики")
    from .solvers import fast, greedy, improved

    t = time.time()
    plan_g = greedy.solve(day)
    problems = plan_g.check_invariants(day)
    if problems:
        bad(f"жадный нарушил инварианты: {problems[:2]}")
    else:
        ok(f"жадный: назначено {plan_g.assigned_count} из 80, "
           f"инварианты чисты — {time.time() - t:.1f} с")

    t = time.time()
    # Бюджет в шагах, а не в секундах: иначе selfcheck печатает каждый раз
    # немного разные числа — на сколько успел поиск, зависит от того, чем
    # занята машина. С фиксированным числом шагов два прогона сравнимы
    # построчно, и расхождение сразу видно.
    params = improved.Params(lns_seconds=0.0, lns_iterations=900)
    plan_i = fast.solve(day, params=params, seed=1)
    problems = plan_i.check_invariants(day)
    if problems:
        bad(f"улучшенный нарушил инварианты: {problems[:2]}")
    else:
        ok(f"улучшенный: назначено {plan_i.assigned_count} из 80, "
           f"инварианты чисты — {time.time() - t:.1f} с")

    if plan_i.assigned_count <= plan_g.assigned_count:
        bad(f"улучшенный назначил не больше жадного "
            f"({plan_i.assigned_count} против {plan_g.assigned_count})")

    # Ускоренная версия обязана давать ровно то же, что медленная: иначе
    # все замеры меряют один солвер, а объясняются другим. Нулевых штрафов
    # для этого мало: продакшен считает с ненулевыми balance_weight и
    # churn_penalty, а они дольше всего прожили в одной реализации из двух.
    # Якорь берём из жадного плана — он детерминирован, поэтому расхождение
    # воспроизводится тем же запуском, а не «иногда».
    def seq_of(plan):
        # Не только маршруты, но и то, чем план себя объявляет. `to_plan`
        # написан в каждой реализации свой (`fast.py` и `improved.py`),
        # и метаданные они собирают порознь — а сверка до 25 сентября
        # сравнивала одни последовательности визитов.
        #
        # Цена дыры измерена: если заставить `fast.to_plan` отдавать
        # `stock=None`, все 22 набора остаются зелёными. А `Plan.stock` —
        # это выключатель инварианта «в машине не больше, чем в ней
        # есть»: одним словом боевому солверу молча снимается проверка
        # оборудования, и ни сверка, ни отметка про оборудование этого
        # не видят (вторая требует, чтобы солвер не бросил, — а с
        # разоружённым инвариантом он и не бросает).
        мета = (plan.duration_factor,
                tuple(sorted((plan.pinned or {}).items())),
                None if plan.stock is None else
                tuple(sorted((e, tuple(sorted(v.items())))
                             for e, v in plan.stock.items())))
        return ({e: [v.order.id for v in r.visits] for e, r in plan.routes.items()},
                мета)

    anchor = {v.order.id: eid for eid, r in plan_g.routes.items() for v in r.visits}

    # Закрепление диспетчером: берём заявки жадного плана и отдаём их
    # СОСЕДУ по списку инженеров — так набор заведомо меняет решение, а
    # не совпадает с тем, что солвер выбрал бы сам. Инженер подбирается
    # из тех, кто вообще может взять заявку: закрепление за неспособным
    # это отдельный случай, и проверяется он не здесь.
    by_id = {o.id: o for o in day.orders}
    pinned = {}
    for oid, eid in list(anchor.items())[:8]:
        годные = [e.id for e in day.engineers if e.can_do(by_id[oid])]
        сосед = next((x for x in годные if x != eid), None)
        if сосед:
            pinned[oid] = сосед

    # Каждый набор отличается от «без штрафов» ровно одним слагаемым —
    # иначе ярлык врёт, а проверка `inert` ниже перестаёт что-либо
    # значить: случай, чей штраф не дошёл ни до одной реализации, всё
    # равно прошёл бы за счёт остальных отличий. Считать от нуля
    # приходится потому, что рабочие значения `balance_weight` и
    # `sla_weight` ненулевые по умолчанию: набор `{balance_weight:
    # BALANCE_WEIGHT}` — это `Params()` поле в поле, а вовсе не
    # «включённый штраф».
    БЕЗ_ШТРАФОВ = {"balance_weight": 0.0, "sla_weight": 1.0,
                   "priority_weights": (1.0, 1.0, 1.0)}

    def тесно(d, extra) -> dict:
        """Запас в машинах вдвое меньше того, что план без него везёт."""
        from .core.domain import equipment_sum
        base = improved.solve(d, params=improved.Params(lns_seconds=0.0, **extra))
        return {e.id: {k: n // 2 for k, n in equipment_sum(
                    v.order for v in base.routes[e.id].visits).items()}
                if e.id in base.routes else {} for e in d.engineers}

    cases = [
        ("без штрафов", БЕЗ_ШТРАФОВ),
        ("штраф за неравномерность",
         {**БЕЗ_ШТРАФОВ, "balance_weight": improved.BALANCE_WEIGHT}),
        ("вес обязательства", {**БЕЗ_ШТРАФОВ, "sla_weight": 2.0}),
        # Приоритета распределения здесь нет нарочно. Он живёт в первом
        # уровне цели, а цель при нулевом бюджете не спрашивают вовсе:
        # сборка ставит всё, что влезает, и приоритет в неё входит только
        # надбавкой к сожалению — она была и раньше. Здесь его набор
        # совпал бы с «без штрафов», и сторож ниже справедливо назвал бы
        # сверку вырожденной. Приоритет сверяется ниже, в поиске.
        ("штраф за смену исполнителя",
         {**БЕЗ_ШТРАФОВ, "anchor": anchor, "churn_penalty": 40.0}),
        # Тяга аварий к раннему началу. В `fast` член начисляется
        # повизитно, в `improved` — суммой по маршруту, и сверка здесь
        # единственное место, где видно, что это одно и то же.
        ("срочность аварий", {**БЕЗ_ШТРАФОВ, "urgency_weight": 30.0}),
        # Закрепление диспетчером. В `fast` оно сужает `capable`, в
        # `improved` отсекает кандидата в цикле — две разные строки, и
        # сверка единственное место, где видно, что они про одно.
        ("закреплено диспетчером", {**БЕЗ_ШТРАФОВ, "pinned": pinned}),
        # Оборудование в машине. Одна проверка `base.fits_stock` на обе
        # реализации, но зовут её из разных мест: `fast` — при отборе
        # опций маршрута, `improved` — в цикле кандидатов. Запас нарочно
        # вдвое меньше того, что везёт план без штрафов: иначе набор
        # совпал бы с базой и сверка на нём была бы вырожденной.
        ("запас оборудования", {**БЕЗ_ШТРАФОВ, "stock": тесно(day, БЕЗ_ШТРАФОВ)}),
        # Запас времени в маршруте — две ручки, и обе до 23 сентября в
        # сверку не входили: `buffer_base` по умолчанию ноль, то есть
        # слагаемое не работало ни в одном наборе, а `buffer_step`
        # оставался своим умолчанием во всех сразу, и ни один набор его
        # не различал. `fast` зовёт `p.buffer_fn(position)` сам, а
        # `improved` передаёт ту же функцию в `base.place`, где её зовут
        # от номера визита: значение общее, а вот от чего его считают —
        # у каждого своё, и сверка единственное место, где видно, что это
        # один и тот же номер.
        ("запас первого визита", {**БЕЗ_ШТРАФОВ, "buffer_base": 20}),
        ("прирост запаса выключен", {**БЕЗ_ШТРАФОВ, "buffer_step": 0.0}),
        # Длительность с запасом — живая настройка демо, до 2,0. В
        # `improved` она уходит параметром в общий `base.place`, в `fast`
        # продублирована руками двумя `int(round(est_minutes * factor))`.
        # Два места, которые обязаны считать одно и то же.
        ("длительность с запасом", {**БЕЗ_ШТРАФОВ, "duration_factor": 2.0}),
        # И два составных: отдельно то, что реально уезжает на демо
        # (`Params()` без аргументов), отдельно все слагаемые разом —
        # штрафы могут быть верны поодиночке и разъехаться вместе.
        ("боевые значения", {}),
        ("всё сразу", {"anchor": anchor, "churn_penalty": 40.0,
                       "urgency_weight": 30.0, "pinned": pinned}),
    ]

    diverged, inert = [], []
    reference = None
    for label, extra in cases:
        slow = improved.Params(lns_seconds=0.0, **extra)
        # Невалидный план солвер не выпускает: он бросает `InvalidPlan`.
        # Для сверки это расхождение с названием набора, а не падение.
        try:
            seq_ref = seq_of(improved.solve(day, params=slow, seed=1))
            seq_new = seq_of(fast.solve(day, params=slow, seed=1))
        except InvalidPlan as exc:
            diverged.append(f"{label}: {str(exc)[:80]}")
            continue
        if seq_ref != seq_new:
            diverged.append(label)
        if reference is None:
            reference = seq_ref
        elif seq_ref == reference:
            inert.append(label)

    if diverged:
        bad(f"конструкция разошлась между реализациями "
            f"({', '.join(diverged)}) — замеры и объяснения относятся "
            f"к разным алгоритмам")
    else:
        ok(f"конструкция плана совпадает визит в визит "
           f"на {len(cases)} наборах параметров")

    # Сверка на параметре, который ничего не меняет, ничего и не проверяет:
    # план обязан отличаться от плана без штрафов, иначе строчка выше
    # зелёная просто потому, что штраф не дошёл ни до одной реализации.
    if inert:
        bad(f"план не меняется от штрафа ({', '.join(inert)}) — "
            f"сверка на нём вырождена")

    # Нулевой бюджет проверяет только конструктор. Весь цикл поиска —
    # выбивание, пересборка, инкрементальные префиксы — до сих пор не
    # сверялся ничем: по секундам это невозможно, быстрая версия успевает
    # на два порядка больше шагов. С бюджетом в итерациях обе делают
    # одинаковое число шагов, и сверяется алгоритм целиком.
    # День меньше основного: на нём те же ветки поиска стоят вчетверо
    # дешевле, а медленная реализация тут — обычная эталонная, не быстрая.
    small = generate_day(seed=4, n_orders=40, n_engineers=6, network=net)
    small_anchor = {v.order.id: eid
                    for eid, r in greedy.solve(small).routes.items() for v in r.visits}
    ITERS = 25
    lns_diverged = []
    # Наборы те же и по тому же правилу, что при нулевом бюджете:
    # отличие от базы ровно в одном слагаемом. Срочность аварий здесь
    # обязательна — это единственное слагаемое, начисляемое в двух
    # реализациях по-разному, и без неё весь цикл поиска на нём не
    # сверялся бы ничем.
    lns_cases = [
        ("без штрафов", БЕЗ_ШТРАФОВ),
        # Приоритет распределения: «Авария → Подключение → Ремонт /
        # Дозаказ». Живёт в первом уровне цели и решает, что останется
        # лежать, — дрейф между реализациями здесь хуже любого другого.
        ("приоритет распределения",
         {**БЕЗ_ШТРАФОВ, "priority_weights": improved.PRIORITY_WEIGHTS}),
        ("штраф за неравномерность",
         {**БЕЗ_ШТРАФОВ, "balance_weight": improved.BALANCE_WEIGHT}),
        ("штраф за смену исполнителя",
         {**БЕЗ_ШТРАФОВ, "anchor": small_anchor, "churn_penalty": 40.0}),
        ("срочность аварий", {**БЕЗ_ШТРАФОВ, "urgency_weight": 30.0}),
        # Закрепление диспетчером. В `fast` оно сужает `capable`, в
        # `improved` отсекает кандидата в цикле — две разные строки, и
        # сверка единственное место, где видно, что они про одно.
        ("закреплено диспетчером", {**БЕЗ_ШТРАФОВ, "pinned": pinned}),
        ("запас оборудования", {**БЕЗ_ШТРАФОВ, "stock": тесно(small, БЕЗ_ШТРАФОВ)}),
        # Уровень «меньше людей». В сборке его нет нарочно — как и
        # приоритета: он живёт во втором уровне цели, а цель при нулевом
        # бюджете не спрашивают, и набор там выходил вырожденным
        # (проверено). Здесь он к месту: единица — строгая лексикография,
        # ноль — уровень выключен, и считают его две реализации из разных
        # мест (`used` против `engineers_used()`).
        ("вес лишнего инженера выключен",
         {**БЕЗ_ШТРАФОВ, "vehicle_weight": 0.0}),
        ("боевые значения", {}),
        ("всё сразу", {"anchor": small_anchor, "churn_penalty": 40.0,
                       "urgency_weight": 30.0}),
    ]
    # Набор, от которого ничего не меняется, ничего и не сверяет — та же
    # ловушка, что у сборки выше, только здесь её до 23 сентября не
    # сторожил никто. След берём парой «план плюс цель»: слагаемое,
    # которое не переставило ни одного визита, но изменило стоимость,
    # до обеих реализаций всё-таки дошло, а стоимость и сверяется.
    lns_inert, эталон = [], None
    for label, extra in lns_cases:
        p_lns = improved.Params(lns_seconds=0.0,
                                lns_iterations=ITERS, **extra)
        rep_ref, rep_new = {}, {}
        try:
            a = improved.solve(small, params=p_lns, seed=4, report=rep_ref)
            b = fast.solve(small, params=p_lns, seed=4, report=rep_new)
        except InvalidPlan as exc:
            lns_diverged.append(f"{label}: {str(exc)[:80]}")
            continue
        # Кроме плана сверяется и значение целевой функции. Одним планом
        # мелкий численный дрейф в инкрементальных префиксах не поймать:
        # он меняет стоимость, но не порядок вставок, и план выходит тот
        # же. Порядок сложения у двух реализаций разный, поэтому равенство
        # тут не побитовое, а с допуском — но допуск на восемь порядков
        # меньше самой дешёвой вставки.
        #
        # Сверяются все три уровня цели. Первые два без третьего не
        # проверяют ничего сверх плана: `obj[0]` — несделанное, `obj[1]` —
        # число непустых маршрутов, и то и другое однозначно следует из
        # плана, сверенного строкой выше. Дрейф живёт ровно в `obj[2]`,
        # ради него проверка и написана; до появления среднего уровня
        # стоимость и была вторым элементом, а при вставке `vehicle_weight`
        # в кортеж индекс не поправили.
        obj_ref, obj_new = rep_ref["objective"], rep_new["objective"]
        if (seq_of(a) != seq_of(b)
                or rep_ref["accepted"] != rep_new["accepted"]
                or rep_ref["iterations"] != rep_new["iterations"]
                or obj_ref[0] != obj_new[0]
                or abs(obj_ref[1] - obj_new[1]) > 1e-6
                or abs(obj_ref[2] - obj_new[2]) > 1e-6):
            lns_diverged.append(label)

        след = (seq_of(a), tuple(obj_ref))
        if эталон is None:
            эталон = след
        elif след == эталон:
            lns_inert.append(label)

    if lns_inert:
        bad(f"поиск не меняется от штрафа ({', '.join(lns_inert)}) — "
            f"сверка на нём вырождена")

    if lns_diverged:
        bad(f"поиск разошёлся между реализациями ({', '.join(lns_diverged)}) — "
            f"инкрементальные пересчёты в fast.py считают не то же самое")
    else:
        ok(f"поиск совпадает шаг в шаг: {ITERS} итераций LNS, те же принятые "
           f"ходы, все три уровня целевой функции, "
           f"{len(lns_cases)} наборов параметров")

    # Приоритет распределения, как его сформулировали организаторы:
    # «Авария → Подключение → Ремонт / Дозаказ. Авария имеет максимальный
    # приоритет. Если аварийная заявка появляется уже в течение рабочего
    # дня, она может стать основанием для изменения ранее сформированного
    # маршрута бригады».
    #
    # Проба построена так, что исход известен заранее. Перегруженный день,
    # все заявки — ремонт; берём ту, что не влезла, но в одиночку кому-то
    # по силам, и делаем её аварией. Прежний план идёт якорем со штрафом
    # за смену исполнителя — «маршрут сформирован, людям позвонили». При
    # равных весах авария обязана остаться лежать: для прежней цели
    # замена ремонта на неё ничего не давала, а штраф делал её невыгодной.
    # При весах приоритета — обязана встать, вытеснив ремонт.
    #
    # Обе половины проверяются вместе: без первой проба не доказывала бы,
    # что на этом дне приоритету вообще есть что менять.
    import copy as _cp

    def _план(d, веса, **extra):
        пп = improved.Params(lns_seconds=0.0, lns_iterations=300,
                             priority_weights=веса, **extra)
        пл = fast.solve(d, params=пп, seed=1)
        if пл.check_invariants(d):
            bad(f"план пробы приоритета невалиден: {пл.check_invariants(d)[:2]}")
        return пл

    РАВНЫЕ = (1.0, 1.0, 1.0)
    тесно = generate_day(seed=3, n_orders=30, n_engineers=2, network=net)
    тесно.orders = [_cp.copy(o) for o in тесно.orders]
    for o in тесно.orders:
        o.priority = 0
    было = _план(тесно, РАВНЫЕ)
    авария = None
    for u in было.unassigned:
        один = _cp.copy(тесно)
        один.orders = [u]
        if _план(один, РАВНЫЕ).assigned_count == 1:
            авария = u
            break
    if авария is None:
        bad("проба приоритета: на дне нет заявки, которую можно сделать аварией "
            "— проба выродилась, её день надо сменить")
    else:
        авария.priority = 2
        якорь_ = {v.order.id: eid for eid, r in было.routes.items() for v in r.visits}
        def _взята(пл):
            return авария.id in {v.order.id for r in пл.routes.values() for v in r.visits}
        равные = _план(тесно, РАВНЫЕ, anchor=якорь_, churn_penalty=40.0)
        боевые = _план(тесно, improved.Params().priority_weights,
                       anchor=якорь_, churn_penalty=40.0)
        if _взята(равные):
            bad("проба приоритета выродилась: авария встаёт и без приоритета, "
                "значит на этом дне ему нечего менять")
        elif not _взята(боевые):
            bad(f"авария {авария.id} не вытеснила ремонт из сложенного маршрута — "
                f"приоритет распределения не работает")
        else:
            ok(f"авария вытесняет ремонт из сложенного маршрута: назначено "
               f"{равные.assigned_count} → {боевые.assigned_count}, меняется "
               f"только что")

    # Выбивание заявки может утащить за собой соседей: без неё инженер
    # выезжает раньше, попадает в час пик и приезжает позже, чем
    # закрывается окно следующей заявки. На синтетическом дне это не
    # случается ни разу — ветка живёт страховкой, и потому не проверена
    # ничем. Проверяем на дне, построенном ровно под этот случай.
    casc = _cascade_day(net)
    ref_state = improved._State(casc, improved.Params(lns_seconds=0.0))
    ref_state.seq["E00"] = list(casc.orders)
    evicted_ref = ref_state.remove(casc.orders[0])

    new_sol = fast._Solution(casc, improved.Params(lns_seconds=0.0))
    new_sol.routes["E00"].seq = list(casc.orders)
    new_sol.routes["E00"].rebuild()
    evicted_new = new_sol.routes["E00"].remove(casc.orders[0])

    ids_ref = [o.id for o in evicted_ref]
    ids_new = [o.id for o in evicted_new]
    if ids_ref != ["B0"] or ids_new != ["B0"]:
        bad(f"каскадное выбивание отработало не так: медленная {ids_ref}, "
            f"быстрая {ids_new}, ожидалось ['B0'] у обеих")
    elif ref_state.seq["E00"] or new_sol.routes["E00"].seq:
        bad("после каскада в маршруте остались визиты, которых там быть не должно")
    else:
        ok("каскад при выбивании: обе реализации одинаково утащили соседа")

    # --- 4б. равномерность загрузки ---
    from .diagnose import balance

    # Сравниваем с планом без штрафа: рабочий вес теперь стоит значением
    # по умолчанию, и `plan_i` уже посчитан с ним. Сравнивать его с самим
    # собой смысла нет.
    rough = fast.solve(day, params=improved.Params(
        lns_seconds=0.0, lns_iterations=900, balance_weight=0.0), seed=1)
    b0, b1 = balance(day, rough), balance(day, plan_i)
    # Требуем улучшения, а не «не ухудшения». Условие было
    # односторонним (`b1 > b0` — красный), и равенство считалось
    # успехом: с выключенным `balance_weight` отметка печатала бы
    # «загрузка выравнивается: Джини 0.293 → 0.293» и была бы зелёной.
    # Порог 0,01 — заметно больше шума округления и заметно меньше того,
    # что даёт рабочий вес (на этом дне 0,293 → 0,150).
    if b1["occupancy_gini"] > b0["occupancy_gini"] - 0.01:
        bad(f"штраф за неравномерность не выравнивает загрузку: "
            f"Джини {b0['occupancy_gini']:.3f} → {b1['occupancy_gini']:.3f} "
            f"(ждём убыли хотя бы на 0,01)")
    else:
        ok(f"загрузка выравнивается: Джини {b0['occupancy_gini']:.3f} → "
           f"{b1['occupancy_gini']:.3f}, занятость "
           f"{b1['occupancy_min'] * 100:.0f}–{b1['occupancy_max'] * 100:.0f}% "
           f"(назначено {plan_i.assigned_count} против {rough.assigned_count})")

    # --- 5. симуляция ---
    step("5. Симуляция дня (300 прогонов)")
    from .core.simulate import monte_carlo

    t = time.time()
    mc_g = monte_carlo(day, plan_g, runs=300, seed=1)
    mc_i = monte_carlo(day, plan_i, runs=300, seed=1)
    ok(f"два прогона по 300 симуляций — {time.time() - t:.1f} с")

    print(f"\n      {'':<22}{'жадный':>10}{'улучшенный':>13}")
    print(f"      {'назначено':<22}{plan_g.assigned_count:>10}{plan_i.assigned_count:>13}")
    print(f"      {'выполнено':<22}{mc_g.done_mean:>10.1f}{mc_i.done_mean:>13.1f}")
    print(f"      {'покрытие дня':<22}{mc_g.coverage * 100:>9.1f}%{mc_i.coverage * 100:>12.1f}%")
    print(f"      {'срывов по окну':<22}"
          f"{mc_g.reasons_mean.get('missed_window', 0):>10.2f}"
          f"{mc_i.reasons_mean.get('missed_window', 0):>13.2f}")
    print()

    if mc_i.coverage <= mc_g.coverage:
        bad("улучшенный не обогнал жадный по покрытию — это ненормально")
    else:
        ok(f"улучшенный впереди на {(mc_i.coverage - mc_g.coverage) * 100:.1f} п.п. "
           f"(на одном дне; значимость даёт bench на 12 днях)")

    if mc_i.done_sd < 1.0:
        bad(f"разброс подозрительно мал (sd {mc_i.done_sd:.2f}): "
            f"похоже, случайность разыгрывается не в каждом прогоне")
    else:
        ok(f"разброс живой: sd {mc_i.done_sd:.2f}, p10 {mc_i.done_p10} … p90 {mc_i.done_p90}")

    top = mc_i.fragile[0] if mc_i.fragile else None
    if top and top[1] > 0.95:
        bad(f"визит {top[0]} срывается в {top[1] * 100:.0f}% прогонов — "
            f"признак предзаписанной случайности")
    elif top:
        ok(f"самый хрупкий визит {top[0]} срывается в {top[1] * 100:.0f}% прогонов")

    # --- 6. подбор смен ---
    step("6. Подбор смен (только прокси, без долгой доводки)")
    from .shiftopt import apply, optimize

    t = time.time()
    sp = optimize(day, restarts=6, seed=1)
    tuned = apply(day, sp.starts)
    plan_t = fast.solve(tuned, params=params, seed=1)
    mc_t = monte_carlo(tuned, plan_t, runs=200, seed=1)
    ok(f"подобрано за {time.time() - t:.1f} с: {sp.describe()}")
    ok(f"покрытие {mc_i.coverage * 100:.1f}% → {mc_t.coverage * 100:.1f}% "
       f"только за счёт графика выхода")

    # --- 7. выбытие инженера посреди дня ---
    step("7. Инженер выбыл посреди дня")
    import random as _random

    from .replan import apply_dropout, carve_remainder, simulate_until

    st = simulate_until(day, plan_i, 13 * 60, _random.Random(1))
    busy = sorted(e for e, ids in st.pending.items() if ids)
    if not busy:
        bad("к 13:00 ни у кого не осталось работы — проверять нечего")
    else:
        gone = busy[0]
        whole = carve_remainder(day, st)
        pool = {o.id for o in whole.orders}

        # Полное выбытие: инженера нет, а его заявки остались в пуле.
        maimed = apply_dropout(whole, gone, None)
        lost = [o for o in st.pending[gone] if o not in pool]
        if any(e.id == gone for e in maimed.engineers):
            bad(f"{gone} выбыл, но остался в списке инженеров")
        elif lost:
            bad(f"заявки выбывшего пропали из пула, а не осели в нём: {lost[:3]}")
        else:
            anchor = {oid: eid for eid, ids in st.pending.items()
                      for oid in ids if eid != gone}
            p = fast.solve(maimed, params=improved.Params(
                lns_seconds=1.0, anchor=anchor,
                churn_penalty=40.0), seed=1)
            problems = p.check_invariants(maimed)
            picked = sum(1 for r in p.routes.values() for v in r.visits
                         if v.order.id in set(st.pending[gone]))
            if problems:
                bad(f"план после выбытия невалиден: {problems[:2]}")
            elif gone in p.routes:
                bad(f"выбывшему {gone} всё равно назначили визиты")
            else:
                ok(f"{gone} выбыл с {len(st.pending[gone])} визитами впереди, "
                   f"{picked} из них подхватили остальные, план валиден")

        # Прокол: инженер на месте, но следующий визит только через 40 минут.
        was = {e.id: e.shift_start for e in whole.engineers}[gone]
        flat = apply_dropout(whole, gone, 40)
        now = {e.id: e.shift_start for e in flat.engineers}[gone]
        p2 = fast.solve(flat, params=improved.Params(
            lns_seconds=1.0), seed=1)
        if now <= was and now != {e.id: e.shift_end for e in flat.engineers}[gone]:
            bad(f"прокол не сдвинул начало смены: было {was}, стало {now}")
        elif p2.check_invariants(flat):
            bad(f"план после прокола невалиден: {p2.check_invariants(flat)[:2]}")
        else:
            ok(f"после прокола {gone} продолжает с {now // 60:02d}:{now % 60:02d} "
               f"оттуда, где встал; план валиден")

    # Статусы «отправлено» и «в пути» — обратная связь организаторов
    # 20 сентября. Отправленный наряд держит исполнителя, как ручное
    # закрепление; к заявке «в пути» человек уже едет, и пересчёт её не
    # трогает вовсе, а сам он освобождается у неё после окончания работ.
    from .journal import Journal

    журнал = Journal(day="1")
    журнал.adopt(plan_i)
    длинные = [(eid, r) for eid, r in plan_i.routes.items() if len(r.visits) >= 2]
    (кто_отпр, м1), (кто_едет, м2) = длинные[0], длинные[1]
    отпр, едет = м1.visits[0].order.id, м2.visits[0].order.id
    выезд = м2.visits[0].arrive - м2.visits[0].travel
    журнал.add(day, {"kind": "dispatched", "order": отпр, "at": 540})
    журнал.add(day, {"kind": "en_route", "order": едет, "at": выезд})
    try:
        журнал.add(day, {"kind": "en_route", "order": м2.visits[1].order.id,
                         "at": выезд + 1})
        отказ = False
    except ValueError:
        отказ = True

    сейчас = выезд + 5
    сост = журнал.state(day, сейчас, plan_i.duration_factor)
    остаток = carve_remainder(day, сост)
    после = fast.solve(остаток, params=improved.Params(
        lns_seconds=0.0, lns_iterations=60, pinned=журнал.pinned()), seed=1)
    где = {v.order.id: eid for eid, r in после.routes.items() for v in r.visits}
    в_пуле = едет in где or едет in {o.id for o in после.unassigned}
    свободен = {e.id: e.shift_start for e in остаток.engineers}[кто_едет]
    ждали = max(м2.visits[0].finish, сейчас)
    держит = ((после.pinned or {}).get(отпр) == кто_отпр
              and где.get(отпр) in (кто_отпр, None))
    if not в_пуле and свободен == ждали and держит and отказ:
        ok(f"«в пути»: {едет} не перераспределяется, {кто_едет} свободен с "
           f"{свободен // 60:02d}:{свободен % 60:02d}; «отправлено» держит "
           f"{отпр} за {кто_отпр}")
    else:
        bad(f"статусы: «в пути» {едет} в пуле {в_пуле}, {кто_едет} свободен "
            f"с {свободен} вместо {ждали}; отправленный держится {держит}; "
            f"второй выезд отвергнут {отказ}")

    # Разыгранный пересчёт: кто выехал к визиту раньше момента пересчёта,
    # тот к нему и едет. Прежде такого инженера возвращали в точку выезда,
    # а визит отдавали в пул — статус «в пути» знал только журнал.
    from .core.domain import Day as _Day, Engineer as _Eng, Order as _Ord

    дальний = max(range(1, len(net.nodes)),
                  key=lambda b: net.travel_minutes(0, b, 9 * 60))
    в_дороге = net.travel_minutes(0, дальний, 9 * 60)
    день_д = _Day(date="в пути", network=net, orders=[_Ord(
        id="Д0", work_type="repair", district="—", lat=0.0, lon=0.0,
        node=дальний, window_start=9 * 60, window_end=15 * 60,
        sla_deadline=22 * 60, priority=0, est_minutes=30)], engineers=[_Eng(
        id="E00", name="—", skills={"repair"}, grade=2, home_node=0,
        home_lat=0.0, home_lon=0.0, shift_start=9 * 60, shift_end=18 * 60)])
    план_д = fast.solve(день_д, params=improved.Params(lns_seconds=0.0,
                                                       lns_iterations=0))
    # Десять минут после выезда: даже самая быстрая дорога — 0,7 от
    # оценки, — а оценка больше двадцати минут, так что он ещё в пути.
    сост_д = simulate_until(день_д, план_д, 9 * 60 + 10, _random.Random(1))
    доиграна = "Д0" in сост_д.done or "Д0" in сост_д.failed
    if в_дороге > 20 and "Д0" not in сост_д.pending_ids and доиграна:
        ok(f"разыгранный пересчёт: выехавший в 9:00 к заявке в {в_дороге:.0f} "
           f"мин пути в 9:10 едет к ней, а не отдаёт её в пул")
    else:
        bad(f"разыгранный пересчёт вернул едущего в точку выезда: заявка в "
            f"пуле {'Д0' in сост_д.pending_ids}, доиграна {доиграна}, "
            f"дорога {в_дороге:.0f} мин")

    # Оборудование при перепланировании — обратная связь 20 сентября.
    # Утром выдали под маршрут; установленное из машины ушло, и заявку с
    # роутером пересчёт отдаёт только тому, у кого роутер остался. Запас
    # здесь нулевой нарочно: с ним любая переданная заявка с прибором
    # нарушала бы ограничение, и сторож мог бы покраснеть.
    from .core.domain import equipment_sum as _нужно
    from .replan import morning_kit, stock_left

    выдано = morning_kit(day, plan_i, reserve=0)
    к_часу = simulate_until(day, plan_i, 13 * 60, _random.Random(1))
    осталось = stock_left(выдано, к_часу, day)
    ушло = sum(sum(v.values()) for v in выдано.values()) - sum(
        sum(v.values()) for v in осталось.values())
    установлено = sum(sum(day.order_by_id(o).equipment.values()) for o in к_часу.done)
    с_прибором = [e for e, ids in к_часу.pending.items()
                  if any(day.order_by_id(o).equipment for o in ids)]
    выбыл = с_прибором[0] if с_прибором else None
    без_ограничения = с_ограничением = None
    if выбыл:
        без_него = apply_dropout(carve_remainder(day, к_часу), выбыл, None)
        свободно = fast.solve(без_него, params=improved.Params(
            lns_seconds=0.0, lns_iterations=60), seed=1)
        # Тот же план, проверенный с запасом в машинах: сколько нарушений
        # дал бы пересчёт, не знающий об оборудовании.
        свободно.stock = осталось
        без_ограничения = sum(1 for p in свободно.check_invariants(без_него)
                              if "не хватает в машине" in p)
        try:
            fast.solve(без_него, params=improved.Params(
                lns_seconds=0.0, lns_iterations=60, stock=осталось), seed=1)
            с_ограничением = 0
        except InvalidPlan as exc:
            с_ограничением = sum(1 for p in exc.problems if "не хватает в машине" in p)
    # И инвариант обязан поймать перегруженную машину сам по себе.
    пустые = _copy.copy(plan_i)
    пустые.stock = {e.id: {} for e in day.engineers}
    поймал = any("не хватает в машине" in p for p in пустые.check_invariants(day))
    if (ушло == установлено > 0 and без_ограничения and с_ограничением == 0
            and поймал):
        ok(f"оборудование: к 13:00 установлено {установлено} приборов; без "
           f"запаса пересчёт после выбытия {выбыл} перегрузил бы "
           f"{без_ограничения} машин, с ограничением — ни одной")
    else:
        bad(f"оборудование: из машин ушло {ушло}, установлено {установлено}; "
            f"нарушений без ограничения {без_ограничения}, с ограничением "
            f"{с_ограничением}; инвариант поймал {поймал}")

    # --- 8. выгрузка и контракт ---
    step("8. Выгрузка для интерфейса")
    tmp = Path(tempfile.mkdtemp(prefix="vrptw_check_"))
    try:
        t = time.time()
        from .export import export_day
        sizes = export_day(day_id="1", out_dir=str(tmp), runs=200,
                           lns_seconds=3.0, tune_shifts=False,
                           save_geometry=False)
        ok(f"четыре формы за {time.time() - t:.1f} с: " +
           ", ".join(f"{n} {s // 1024} КБ" for n, s in sizes.items()))

        from .validate_fixtures import check
        problems = check(tmp)
        if problems:
            bad(f"контракт нарушен ({len(problems)}): {problems[:2]}")
        else:
            ok("контракт соблюдён: заявки не теряются, окна и смены не нарушены, "
               "explain и simulation сходятся с планом")

        # «Дешевле всех остальных из 1 подходящих» — сравнение не с кем; и
        # падеж: «из 21 подходящего», а не «подходящих».
        import re as _re
        объяснения = json.loads(next(tmp.rglob("explain*.json")).read_text(encoding="utf-8"))
        сводки = [o["summary"] for o in объяснения["orders"].values()]
        кривые = [s for s in сводки
                  if (m := _re.search(r"из (\d+) (подходящих|подходящего)$", s))
                  and (m.group(1) in ("0", "1") or (m.group(2) == "подходящего")
                       != (int(m.group(1)) % 10 == 1 and int(m.group(1)) % 100 != 11))]
        единственных = sum("единственный, кому" in s for s in сводки)
        # «Единственный» — только если подходящий один и это назначенный.
        не_тот = [oid for oid, o in объяснения["orders"].items()
                  if "единственный, кому" in o["summary"]
                  and [c["verdict"] for c in o["candidates"]
                       if c["verdict"] in ("chosen", "feasible")] != ["chosen"]]
        кривые += не_тот
        if кривые or not единственных:
            bad(f"объяснения: кривые сводки {кривые[:2]}, «единственный» {единственных} "
                f"(в синтетическом дне первом такие заявки есть)")
        else:
            ok(f"объяснения: «единственный» у {единственных}, «из N подходящих» — "
               f"с N > 1 и в падеже")
    finally:
        shutil.rmtree(tmp, ignore_errors=True)

    состояние_после = _боевое_состояние(боевой_каталог)
    if состояние_после != состояние_до:
        bad(f"проверка тронула боевое состояние в {боевой_каталог}: было "
            f"{состояние_до[0]} ({состояние_до[1]} файлов), стало "
            f"{состояние_после[0]} ({состояние_после[1]}) — где-то путь "
            f"зашит мимо `state.py`")
    else:
        ok(f"боевое состояние не тронуто: {состояние_до[1]} файлов, "
           f"отпечаток {состояние_до[0]}")

    данные_после = _отпечаток_данных()
    if данные_после != данные_до:
        bad(f"проверка изменила vrptw/data: было {данные_до[0]} "
            f"({данные_до[1]} файлов), стало {данные_после[0]} "
            f"({данные_после[1]}). Смотрите `git status vrptw/data`")
    else:
        ok(f"vrptw/data не тронут проверкой: {данные_до[1]} файлов, "
           f"отпечаток {данные_до[0]}")

    не_для_windows, сверено_команд = _команды_для_обеих_систем()
    if не_для_windows or not сверено_команд:
        bad(f"документы советуют команды, которых на Windows нет: "
            f"{не_для_windows[:2]}, сверено документов {сверено_команд}")
    else:
        ok(f"команды в документах есть и для Windows "
           f"(сверено документов {сверено_команд})")

    порт_врёт, случаев = _объяснение_порта()
    if порт_врёт or not случаев:
        bad(f"сообщение о порте путает причины: {порт_врёт[:2]}, "
            f"случаев {случаев}")
    else:
        ok(f"занятый порт и порт без прав объясняются по-разному "
           f"({случаев} построенных случая)")

    # Последняя отметка — про само число отметок, поэтому в сверку идёт
    # `отметок + 1`: своя ещё не напечатана, а в документах стоит итог.
    врут, сверено = _отметки_в_документах("python3 -m vrptw.selfcheck", отметок + 1)
    if врут or not сверено:
        bad(f"документы врут про число отметок: {врут}, сверено мест {сверено}")
    else:
        ok(f"документы называют верное число отметок: {отметок + 1} "
           f"(сверено мест {сверено})")

    # --- итог ---
    print("\n" + "─" * 62)
    if failures:
        print(f"✗ Проблем: {len(failures)}")
        for f in failures:
            print(f"    {f}")
        return 1

    print(f"✓ Стенд собран правильно. Всё заняло {time.time() - t_all:.0f} с.")
    print("\n  Дальше можно запускать длинные замеры — см. README, раздел «Запуск».")
    return 0


if __name__ == "__main__":
    sys.exit(main())
