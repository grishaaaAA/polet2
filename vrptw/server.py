...
def main():
    ...
    try:
        ...
        if exc.errno == errno.EADDRINUSE:
            print(f"  ✗ порт {port} уже кто-то слушает.")
            print(f"    Взять другой:  python3 -m vrptw.server {port + 1} {дни}")
            print(f"    Кто держит:    lsof -ti :{port}   "
                  f"(на Windows — netstat -ano | findstr :{port})")
            print(f"    Если это прежний стенд — остановите его и повторите.")
        elif exc.errno in (errno.EACCES, errno.EPERM):
            print(f"  ✗ порт {port} не отдают без особых прав: "
                  f"номера меньше 1024 заняты системой.")
            print(f"    Возьмите номер побольше:  "
                  f"python3 -m vrptw.server 8000 {дни}")
        else:
            print(f"  ✗ порт {port} открыть не вышло: {exc}")
        raise SystemExit(2)
