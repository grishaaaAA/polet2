...
def _чем_считано(запись: dict) -> dict:
    """Сказать, если сохранённый план посчитан не сегодняшним движком.

    Пустой словарь — движок тот же или запись старше этой проверки.

    План сохранённого пересчёта хранится файлом и отдаётся как есть:
    пересчёт живёт по секундам, новый счёт дал бы другой план, а архив
    отвечает на вопрос «что мы видели, когда принимали решение».
    Проверять его инвариантами сегодняшним кодом значит однажды потерять
    читаемость архива — после правки правил он перестанет открываться.
    Поэтому не проверяем, а говорим: вот этим движком считано, вот этот
    сейчас. Решает человек.
    """
    тогда = запись.get("код_движка")
    сейчас = отпечаток_кода()
    if not тогда:
        return {"движок": {"тогда": None, "сейчас": сейчас,
                           "словами": "запись сделана до того, как движок "
                                      "стал помнить свой отпечаток; каким "
                                      "кодом посчитано — неизвестно"}}
    if тогда == сейчас:
        return {}
    return {"движок": {"тогда": тогда, "сейчас": сейчас,
                       "словами": f"план посчитан движком {тогда}, сейчас "
                                  f"{сейчас}: числа могли измениться. "
                                  f"Чтобы увидеть сегодняшние — повторите "
                                  f"пересчёт и сохраните заново"}}
