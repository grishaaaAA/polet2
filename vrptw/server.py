...
def _чем_считано(запись: dict, *, план_с_диска: bool) -> dict:
    """Сказать, если запись архива посчитана не сегодняшним движком.

    Пустой словарь — движок тот же, говорить не о чем.

    Устаревает разное, и врать об этом нельзя. Сохранённый пересчёт
    хранится файлом и отдаётся как есть: пересчёт живёт по секундам,
    новый счёт дал бы другой план, а архив отвечает на вопрос «что мы
    видели, когда принимали решение». Проверять его инвариантами
    сегодняшним кодом значит однажды потерять читаемость архива — после
    правки правил он перестанет открываться. Поэтому не проверяем, а
    говорим: вот этим движком считано, вот этот сейчас.

    У обычного расчёта план, наоборот, **пересчитывается** при каждом
    открытии, то есть он всегда сегодняшний. Устаревает там другое —
    `summary`, сохранённый в момент создания: это те числа, что стоят на
    карточке в списке. Сказать про такую запись «план посчитан движком
    X» было бы неправдой, поэтому у неё своя фраза.

    До 25 сентября эта функция звалась ровно из одного места — с ветки
    сохранённого пересчёта, у которого файл плана есть, — и на боевом
    архиве не срабатывала ни разу: там десять записей, все обычные, и
    `код_движка` у всех пустой.
    """
    тогда = запись.get("код_движка")
    сейчас = отпечаток_кода()
    if тогда == сейчас:
        return {}

    неизвестно = ("запись сделана до того, как движок стал помнить свой "
                  "отпечаток, — каким кодом её считали, неизвестно")
    if план_с_диска:
        словами = (
            f"{неизвестно}; план показан таким, каким его сохранили"
            if not тогда else
            f"план посчитан движком {тогда}, сейчас {сейчас}: числа могли "
            f"измениться. Чтобы увидеть сегодняшние — повторите пересчёт "
            f"и сохраните заново")
    else:
        словами = (
            f"{неизвестно}; сам план пересчитан сегодняшним движком, а "
            f"числа на карточке остались от записи"
            if not тогда else
            f"числа на карточке сохранены движком {тогда}, сейчас "
            f"{сейчас}: сам план пересчитан сегодняшним, и с карточкой он "
            f"может расходиться. Создайте расчёт заново, чтобы числа "
            f"сошлись")
    return {"движок": {"тогда": тогда, "сейчас": сейчас,
                       "план_с_диска": план_с_диска, "словами": словами}}
...
# Бюджет поиска для плана дня — в шагах, а не в секундах на стене.
#
# Секунды измеряют продукт: в диспетчерской важна задержка ответа. Но
# число, снятое по секундам, зависит от машины и от того, что на ней ещё
# крутится. 23 сентября это дошло до самого плана: пересчёт готовых на
# той же машине и том же коде дал юго-востоку 73 визита вместо 72, итог
# по трём участкам 191 → 192. Проверяющий, пересчитав готовые у себя,
# получил бы третье число, и сверить его с нашими документами было бы
# нечем.
#
# 1500 — не круглое число с потолка. Столько шагов берут замеры
# (`замеры-22-09-вечер/база-против-нас-интервалы.py`), откуда взяты все
# числа в README и ФТ: теперь экран и документы считают одним и тем же.
# Это же и честный перевод прежних восьми секунд — замерено:
# на дне из 80 заявок восемь секунд дают 1623 шага, а качество выходит на
# плато уже после ~200 (журнал, 7 сентября). На зонах 1500 шагов заодно
# быстрее: восток 3,4 с, юго-восток 5,4 с, югоцентр 2,8 с против восьми.
LNS_ITERATIONS = 1500


def _params(settings: Settings, seconds: float | None = None,
            **extra) -> improved.Params:
    """Параметры солвера по текущим настройкам пользователя.

    Без `seconds` — бюджет в шагах: так считается план дня, и число
    обязано выйти одинаковым у всех. С `seconds` — бюджет по часам: так
    зовёт пересчёт, где обещана задержка ответа.
    """
    if seconds is None:
        return settings.params(lns_seconds=0.0,
                               lns_iterations=LNS_ITERATIONS, **extra)
    return settings.params(lns_seconds=seconds, **extra)


def _бюджет(ident: str) -> dict:
    """Чем ограничить поиск на этом дне.

    Зоны и синтетика — шагами: их числа мы публикуем, и они обязаны выйти
    одинаковыми у всех. Чужая выгрузка — секундами: заявок в ней может
    быть сколько угодно, а шаг на большом дне стоит дорого. Замерено:
    80 заявок — 1500 шагов за 7 с, 200 заявок — за 22 с, 400 — за 68 с,
    тогда как восемь секунд на тех же днях дают 1623, 547 и 183 шага.
    Своей выгрузке мы обещаем ответ, а не воспроизводимость числа,
    которого нигде не печатаем.
    """
    return {"seconds": LNS_SECONDS} if uploads.это_выгрузка(ident) else {}
...
class Stand:
    ...
    def day_and_plan(self, ident: str, st: Settings | None = None):
        ...
        with self._замок_дня(ident):
            ...
            if plan is None:
                начало = time.time()
                plan = fast.solve(day, params=_params(st, **_бюджет(ident)),
                                  seed=rng)
    ...
    def _формы(self, ident: str, fresh: bool = False,
        ...
        if not fresh and откуда is not None and (откуда / path.name).exists():
            ...
            return forms
        params = _params(st, **_бюджет(ident))
    ...
    def with_tuned_shifts(self, ident: str) -> dict:
        ...
        starts, _ = refine(day, starts, solver_seed=rng,
                           sim_seed=rng + 7000, rounds=2, shift_len=None)
        tuned = apply_shifts(day, starts, shift_len=None)
        plan = fast.solve(tuned, params=_params(self.settings, **_бюджет(ident)),
                          seed=rng)
    ...
    def _add_run(self, ident: str, forms_or_plan: dict, note: str = "",
        ...
        def дописать(текущие: list) -> dict:
            ...
            запись = {
                ...
                # Каким движком посчитано. Настройки в записи были всегда,
                # а кода не было: сохранённый пересчёт хранится файлом и
                # отдаётся как есть, и после правки движка он открывался
                # как сегодняшний, хотя считан прежним. Инвариантами его
                # не проверяем нарочно — архив обязан остаться читаемым, —
                # но умолчать о том, что он из другого кода, нельзя.
                "код_движка": отпечаток_кода(),
                "factory": factory,
    ...
    def staffing(self, ident: str, base: str | None = None) -> dict:
        ...
        with self._замок_дня("staffing:" + ident):
            ...
            ответ = сколько_нужно(day, _params(st, **_бюджет(ident)))
    ...
    def list_runs(self) -> dict:
        ...
        # `движок` кладётся и сюда. Контракт советует показать его рядом
        # с карточкой, а карточки берутся отсюда: без этого поля пометить
        # их было нечем, пришлось бы открывать каждую запись.
        with self._lock:
    ...
    def run_forms(self, run_id: str) -> dict:
        ...
        if замысел and сохранённый.exists():
            return {"run": {**запись, "forms": self._формы_записи(запись),
                            **_чем_считано(запись, план_с_диска=True)},
        ...
        if замысел:
            ...
            return {"run": {**запись, "forms": self._формы_записи(запись),
                            **_чем_считано(запись, план_с_диска=False)},
                    "plan": план}

        forms = self.forms(запись["day"], settings=st)
        return {"run": {**запись, "forms": self._формы_записи(запись),
                        **_чем_считано(запись, план_с_диска=False)}, **forms}
...
class Handler(BaseHTTPRequestHandler):
            ...
            if path == "/api/days":
                ...
                return self._send({
                    ...
                    "uploads": [{"day": в["day"], "title": в["title"],
                                ...
                                for в in выгрузки],
                    "note": "day — зона обслуживания (настоящие данные "
    ...
    def _index(self) -> dict:
        return {
            ...
            "endpoints": {
                "GET /api/days": "какие дни готовы: зоны обслуживания, выгрузки и синтетика",
...
class _СчётОтметок:
    """Считает напечатанные отметки, ничего не меняя в выводе.

    В `check` отметки печатаются россыпью обычными `print`, общего
    счётчика у них нет: `runs_ok` считает только свой раздел и 25
    сентября дал 48 там, где напечатано было 79. Поэтому считаем ровно
    то, что видит человек, — строки, начинающиеся с отметки. Итоговая
    строка прогона начинается без отступа и сюда не попадает; из-за неё
    и разъехались числа в документах, когда их считали `grep -c '✓'`.
    """
    ...
    def write(self, текст: str) -> int:
        строки = (self._хвост + текст).split("\n")
        self._хвост = строки.pop()
        self.отметок += sum(1 for с in строки if с.startswith("  ✓"))
        return self._поток.write(текст)

    def flush(self):
        self._поток.flush()

    def __getattr__(self, имя):
        return getattr(self._поток, имя)
...
def check():
    ...
    try:
        ...
        # Обычный расчёт — для проверки ниже: у него план пересчитывается
        # при открытии, и устаревают не маршруты, а числа карточки.
        обычный_с, _ = post("/api/runs", {"day": "восток", "note": "обычный"})
        with STAND._lock:
        ...
        # Пока движок тот же, про движок молчим.
        молчит = "движок" not in откр_с["run"]
        # А после правки движка — обязаны сказать, но план всё равно
        # отдать: архив отвечает на вопрос «что мы видели тогда», и
        # проверять его сегодняшними инвариантами значит однажды его
        # потерять. Поэтому не проверка, а предупреждение.
        глобально = globals()
        настоящий_отпечаток = глобально["отпечаток_кода"]
        глобально["отпечаток_кода"] = lambda: "0000правка"
        try:
            иной, _ = get(f"/api/runs/{сохр_с['id']}")
        finally:
            глобально["отпечаток_кода"] = настоящий_отпечаток
        сказал = (иной["run"].get("движок", {}).get("тогда")
                  == сохр_с.get("код_движка")
                  and иной["run"]["движок"]["сейчас"] == "0000правка"
                  and иной["run"]["движок"]["план_с_диска"] is True
                  and bool(иной.get("plan", {}).get("routes")))

        # То же самое, но у ОБЫЧНОГО расчёта и в списке. До 25 сентября
        # `_чем_считано` звалась ровно из одной ветки — с сохранённого
        # пересчёта, — и на боевом архиве не срабатывала ни разу: там все
        # записи обычные. Проверяем обе стороны: и что говорит, и что
        # говорит своё — у обычного расчёта план пересчитывается, значит
        # устарел не он, а числа карточки.
        глобально["отпечаток_кода"] = lambda: "0000правка"
        try:
            обычный_иной, _ = get(f"/api/runs/{обычный_с['id']}")
            список_иной, _ = get("/api/runs")
        finally:
            глобально["отпечаток_кода"] = настоящий_отпечаток
        д = обычный_иной["run"].get("движок") or {}
        в_списке = [r for r in список_иной["runs"] if r.get("движок")]
        сказал_обычный = (д.get("сейчас") == "0000правка"
                          and д.get("план_с_диска") is False
                          and "карточк" in д.get("словами", "")
                          and bool(обычный_иной.get("plan", {}).get("routes"))
                          and len(в_списке) == len(список_иной["runs"]))
        if сказал_обычный:
            print(f"  ✓ {'обычный расчёт про чужой движок':<38}"
                  f"говорит и в списке, и при открытии; план отдаёт")
            runs_ok += 1
        else:
            print(f"  ✗ обычный расчёт про чужой движок: {д.get('словами', 'молчит')[:60]}, "
                  f"в списке помечено {len(в_списке)} из {len(список_иной['runs'])}")
            failed += 1
        if not вызовы_с and те_же_с and молчит and сказал:
            print(f"  ✓ {'сохранённый пересчёт: перезапуск':<38}"
                  f"тот же, без счёта; про чужой движок говорит, план отдаёт")
            runs_ok += 1
        else:
            print(f"  ✗ сохранённый пересчёт после перезапуска: солвер звали "
                  f"{len(вызовы_с)} раз, маршруты те же {те_же_с}, про свой "
                  f"движок молчит {молчит}, про чужой говорит {сказал}")
            failed += 1
        try:
    ...
    try:
        ...
        #
        # Откуда брать тройку. Раньше брали из `.vrptw-cache` — и проверка
        # падала ровно в том случае, ради которого написана. План попадает
        # в кэш, только если его СЧИТАЛИ: `day_and_plan` пишет туда в ветке
        # `if plan is None`. Когда план уже лежит в `vrptw/data/готовые/`,
        # он читается оттуда и в кэш не переписывается — а это и есть
        # состояние проверяющего на свежем клоне и наше собственное после
        # каждого `server готовые`. Поэтому ищем файл там, где он есть, и
        # начинаем с готовых репозитория: именно их получит проверяющий.
        ключ = ключ_кэша(STAND.settings)
        тройка = {}
        for вид in ("plan", "day", "staffing"):
            имя = f"{вид}-{slug('восток')}-{ключ}.json"
            откуда = next((к / имя for к in (свои_готовые, свой_кэш)
                           if (к / имя).is_file()), None)
            if откуда is None:
                raise FileNotFoundError(
                    f"{имя} нет ни в готовых ({свои_готовые}), ни в кэше "
                    f"({свой_кэш}); прогрейте день или положите готовые")
            тройка[имя] = откуда
        CACHE_DIR = Path(tempfile.mkdtemp(prefix="vrptw-check-пустой-"))
        READY_DIR = Path(tempfile.mkdtemp(prefix="vrptw-check-готовые-"))
        for имя, откуда in тройка.items():
            shutil.copy2(откуда, READY_DIR / имя)
        источник = "готовых репозитория" if all(
            п.parent == свои_готовые for п in тройка.values()) else "кэша"
        вызовы.clear()
        третий = Stand()
        формы_3 = третий.forms("восток")
        третий.staffing("восток")

        # Сравниваем содержимое, а не время генерации. `plan.meta.
        # generated_at` пишется в момент расчёта: у плана, прочитанного из
        # готовых репозитория, там час прогрева, у плана в памяти — время
        # этого прогона. Они обязаны различаться, и сравнивать их значит
        # проверять часы.
        #
        # До 25 сентября это сходилось случайно: тройку брали из кэша,
        # записанного в этом же прогоне, и отметки совпадали. Как только
        # готовые стали подходить по отпечатку — а это и есть состояние
        # проверяющего, — тройка пошла из них, и проверка упала, хотя
        # планы совпадали визит в визит.
        def _без_времени(формы):
            очищенные = json.loads(json.dumps(формы))
            мета = очищенные.get("plan", {}).get("meta")
            if isinstance(мета, dict):
                мета.pop("generated_at", None)
            return json.dumps(очищенные, sort_keys=True, ensure_ascii=False)

        те_же = _без_времени(формы_3) == _без_времени(STAND.forms("восток"))
        if not вызовы and те_же:
            print(f"  ✓ {'готовые планы: считать не пришлось':<38}"
                  f"формы те же, файлы из {источник}")
            runs_ok += 1
        else:
    ...
    # Число отметок, которое обещают документы, обязано совпадать с
    # настоящим. Считать его глазами или `grep ✓` нельзя: греп ловит и
    # итоговую строку, и обе цифры так и разъехались на единицу.
    # Сверка живёт в `selfcheck`, здесь только вызов — одно правило на
    # обе проверки. В сверку идёт `runs_ok + 1`: своя отметка ещё не
    # напечатана, а в документах стоит итог.
    from .selfcheck import _отметки_в_документах
    врут, сверено = _отметки_в_документах("python3 -m vrptw.server check",
                                          счёт.отметок + 1)
    if врут or not сверено:
        print(f"  ✗ документы врут про число отметок: {врут}, "
              f"сверено мест {сверено}")
        failed += 1
    else:
        print(f"  ✓ документы называют верное число отметок: "
              f"{счёт.отметок + 1} (сверено мест {сверено})")
    sys.stdout = счёт._поток
...
def объяснить_порт(port: int, дни: str, exc: OSError) -> list[str]:
    ...
    Вынесена из `main` ради проверяемости. Ветку «нет прав» на этой
    машине красной увидеть нечем: порт 80 здесь отдаётся обычному
    пользователю, и до 25 сентября она была написана, но ни разу не
    сработала. Теперь её зовёт `selfcheck._объяснение_порта` с
    подставленным `errno` — построенный случай по правилу 8.

    Возвращает строки, а не печатает: так их можно сверить.
    """
    if exc.errno == errno.EADDRINUSE:
        return [
            f"  ✗ порт {port} уже кто-то слушает.",
            f"    Взять другой:  python3 -m vrptw.server {port + 1} {дни}",
            f"    Кто держит:    lsof -ti :{port}   "
            f"(на Windows — netstat -ano | findstr :{port})",
            f"    Если это прежний стенд — остановите его и повторите.",
        ]
    if exc.errno in (errno.EACCES, errno.EPERM):
        return [
            f"  ✗ порт {port} не отдают без особых прав: "
            f"номера меньше 1024 заняты системой.",
            f"    Возьмите номер побольше:  "
            f"python3 -m vrptw.server 8000 {дни}",
        ]
    return [f"  ✗ порт {port} открыть не вышло: {exc}"]
...
def main():
    ...
    # Порт занимаем ДО сборки сети. Раньше сервер создавался после неё, и
    # занятый порт выяснялся через несколько секунд ожидания — голым
    # трейсбеком `OSError: [Errno 48]`, без единого слова о том, что
    ...
    # Советуем тот день, который прогревается, а не первый попавшийся.
    # Раньше здесь стояло `day=1`: синтетический день, готовых планов на
    # него нет, и единственная предложенная команда считалась с нуля —
    # проверяющий делал ровно то, что написано, и ждал.
    print(f"  проверить:    curl "
          f"'http://localhost:{port}/api/plan?day={seeds[0]}'")
    print(f"  (прогревается: {', '.join(map(str, seeds))}; другой день "
          f"движок посчитает по запросу, это дольше)\n")
