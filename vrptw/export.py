...
from .core.domain import (EQUIPMENT, SKILL_TITLES, TRANSPORT, WORK_TYPES, Day, Plan,
                          equipment_sum, equipment_text, too_far)
...
def _время(minutes: int) -> str:
    """Минуты от полуночи словами: «10:00».

    В формах времена остаются числами (CONTRACT.md), форматирует их
    интерфейс. Словами время нужно только здесь — внутри причины, которую
    диспетчер читает как фразу.
    """
...
def _самый_ранний_приезд(day: Day, order, кандидаты):
    """
    Кто из них окажется на месте раньше всех — и когда.

    Оценка сверху по досягаемости: маршрут считается пустым, выезд — с
    того момента, когда инженер свободен, и оттуда, где он есть. В
    утреннем плане это начало смены и дом, на остатке дня —
    «когда освободится» и «где сейчас»: `carve_remainder` кладёт то и
    другое в те же поля. Настоящий маршрут приезд может только
    отодвинуть, поэтому «даже так не успевает» — утверждение обо всех
    вариантах сразу, а не об одном.

    `None`, если ни одному из них перегон не по силам транспорту.
    """
    net = day.network
    приезды = []
    for e in кандидаты:
        # Пешком через полгорода не ходят: перегон длиннее предела
        # транспорта временем не отсекается, его отсекает `too_far`.
        if too_far(net, e.home_node, order.node, e.transport):
            continue
        ехать = int(round(net.travel_minutes(
            e.home_node, order.node, e.shift_start, e.transport)))
        # `e.id` в ключе — чтобы при равном времени сравнение не дошло до
        # самого `Engineer`: он не упорядочен, и `min` упал бы.
        приезды.append((e.shift_start + ехать, e.id, e))
    if not приезды:
        return None
    приезд, _, кто = min(приезды)
    return приезд, кто, net.road_km_between(кто.home_node, order.node)
...
def _why_unassigned(day: Day, order, pinned: dict | None = None,
    ...
    if not в_окне:
        return (f"окно {_время(order.window_start)}–{_время(order.window_end)} "
    ...
    # Досягаемость. Прежде отсюда сразу шло «у всех это время занято
    # более срочными заявками», и про дальнего инженера это было
    # враньём. На юго-востоке авария в 7:00 в Москве получала
    # «подходящих инженеров 1, но у всех это время занято», хотя
    # единственный аварийщик участка (Белов, свободен с 8:00) выезжает
    # из Каширы: 107 км, 199 минут, на месте в 11:19 при окне до 10:00.
    # Он не занят — он далеко, и диспетчеру это разные новости.
    # Занятость лечится переброской заявок, расстояние не лечится ничем,
    # кроме человека, посаженного ближе.
    #
    # Проверяется только приезд. Случай «доехал, но работа не помещается
    # до конца смены» модель допускает, однако на трёх зонах заказчика он
    # не встретился ни разу — ни на одной незанятой заявке утренних
    # планов, ни на авариях каждого часа с 6:00 до 20:00. Ветку, которую
    # нечем увидеть красной, не заводим.
    ранний = _самый_ранний_приезд(day, order, с_прибором)
    if ранний is None:
        # Все подходящие — пешеходы и велосипедисты, а ехать дальше, чем
        # они ездят. Это не «не успел»: такой перегон им не по силам ни в
        # какое время.
        return ("ни один подходящий инженер туда не доберётся: перегон "
                "длиннее того, на что годится их транспорт")
    приезд, кто, км = ранний
    if приезд > order.window_end:
        return (f"ни один подходящий инженер не успевает доехать: окно "
                f"закрывается в {_время(order.window_end)}, а раньше всех — "
                f"{кто.name} ({км:.0f} км, на месте в {_время(приезд)})")
...
def export_day(day_id: str = "1", out_dir: str = "fixtures", runs: int = 300,
               lns_seconds: float = 8.0, tune_shifts: bool = True,
               save_geometry: bool = True) -> dict:
    """
    ...
    `save_geometry=False` — не складывать добытые ломаные рядом с
    данными. Нужно проверкам: они гоняют выгрузку ради самой выгрузки, а
    план считают по бюджету в секундах, и маршруты от запуска к запуску
    выходят чуть разные. При интернете каждая такая проверка дописывала в
    `vrptw/data/moscow_geometry.json` горсть перегонов — репозиторий
    становился грязным после команды, которая ничего не меняет.
    """
    ...
    # данными, чтобы следующий запуск обошёлся без интернета. Проверки
    # этого не делают: добытое живёт в памяти сети до конца процесса, а
    # файл в репозитории остаётся каким был.
    if save_geometry and hasattr(net, "save_geometry"):
        net.save_geometry()
