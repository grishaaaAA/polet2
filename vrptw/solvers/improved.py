...
# авария имеет максимальный». Это наше допущение на основе обратной связи
# организаторов 20 сентября, текст которой не сохранён; в стенограмме
# трёхступенчатого порядка нет. Разбор — в `core/load.py`, над
# `ПРИОРИТЕТ_BK`.
...
class _State:
    ...
    def objective(self) -> tuple[float, float, float]:
        ...
        for eng_id in self.seq:
            ...
            if v is None:
                # Невалидное состояние: последовательность этого инженера
                # не расписывается. Бесконечность, а не число: раньше
                # здесь стояло 10 000, ровно столько же весит одна
                # нераспределённая авария (`PRIORITY_WEIGHTS[2]`), и план,
                # оставивший лежать две аварии, стоил 20 000 — то есть
                # дороже невалидного. Отжиг выбрал бы невалидный.
                #
                # На трёх зонах ветка не срабатывает ни разу (4503 вызова
                # `objective`, 0 невалидных): `recreate` вставляет только
                # туда, где расписание сходится. Поэтому правка ничего не
                # двигает сегодня и убирает ловушку на завтра.
                #
                # В `fast` этой ветки нет и не должно быть: он ведёт
                # маршруты пошагово и невалидного состояния не держит
                # вовсе, а `improved` пересчитывает расписание от
                # последовательности и обязан уметь сказать «не вышло».
