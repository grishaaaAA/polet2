...
class Plan:
    ...
    def check_invariants(self, day) -> list[str]:
        ...
        # Закрепление диспетчером. Правило 2 называет `pinned` наравне с
        # `duration_factor` как то, чем план «сам помнит, чем построен»,
        # — а инвариант до 25 сентября к нему не обращался ни разу.
        # Проверено: заявку, стоящую у E00, можно было объявить
        # закреплённой за E01, и `check_invariants` возвращал ноль.
        #
        # Держало обещание только устройство солвера, при том что оно
        # самое громкое перед диспетчером: контракт говорит, что у
        # третьего заявка «не может оказаться ни при каком пересчёте».
        # Ошибку, общую для обеих реализаций, и план, прочитанный с
        # диска, сверка `fast`/`improved` не поймала бы.
        #
        # Закрепление — запрет, а не обещание места: заявка либо у
        # названного инженера, либо ни у кого. Второе законно — ему
        # может не хватить смены, навыка или прибора.
        for oid, eng_id in (self.pinned or {}).items():
            у_кого = кто_везёт.get(oid)
            if у_кого is not None and у_кого != eng_id:
                problems.append(f"{oid}: закреплена за {eng_id}, "
                                f"а стоит у {у_кого}")
